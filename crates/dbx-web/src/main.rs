...
fn first_config_value(primary: Option<String>, legacy: Option<String>) -> Option<String> {
    primary.filter(|value| !value.trim().is_empty()).or_else(|| legacy.filter(|value| !value.trim().is_empty()))
}

fn config_value(primary: &str, legacy: &str) -> Option<String> {
    first_config_value(std::env::var(primary).ok(), std::env::var(legacy).ok())
}

fn web_body_limit_bytes() -> usize {
    const DEFAULT_MB: usize = 1024;
    let mb = config_value("PANDB_MAX_UPLOAD_MB", "DBX_MAX_UPLOAD_MB")
...
fn web_agent_dir(data_dir: &std::path::Path) -> std::path::PathBuf {
    web_agent_dir_from_env(data_dir, config_value("PANDB_AGENT_DIR", "DBX_AGENT_DIR"))
}
...
fn normalize_public_base_path(value: Option<String>) -> String {
    ...
    if trimmed.chars().any(|ch| ch.is_ascii_control() || ch.is_ascii_whitespace() || matches!(ch, ';' | ',')) {
        panic!("PANDB_PUBLIC_BASE_PATH contains invalid characters");
    }
    if trimmed.is_empty() {
        "/".to_string()
    } else {
...
async fn main() {
    ...
    let data_dir = config_value("PANDB_DATA_DIR", "DBX_DATA_DIR").map(std::path::PathBuf::from).unwrap_or_else(|| {
        ...
        let pandb_data_dir = home.join(".pandb-web");
        let legacy_data_dir = home.join(".dbx-web");
        if pandb_data_dir.exists() || !legacy_data_dir.exists() {
            pandb_data_dir
        } else {
            legacy_data_dir
        }
    ...
    let password_disabled = config_value("PANDB_DISABLE_PASSWORD", "DBX_DISABLE_PASSWORD")
        .map(|v| matches!(v.trim().to_lowercase().as_str(), "1" | "true" | "yes" | "on"))
        .unwrap_or(false);

    let password_hash = if password_disabled {
        None
    } else if let Some(pw) = config_value("PANDB_PASSWORD", "PANDB_WEB_PASSWORD")
        .or_else(|| config_value("DBX_PASSWORD", "DBX_WEB_PASSWORD"))
    {
    ...
    let public_base_path = normalize_public_base_path(config_value("PANDB_PUBLIC_BASE_PATH", "DBX_PUBLIC_BASE_PATH"));
    ...
    // Static file serving
    let static_dir = config_value("PANDB_STATIC_DIR", "DBX_STATIC_DIR");
    ...
    } else if config_value("PANDB_PASSWORD", "PANDB_WEB_PASSWORD")
        .or_else(|| config_value("DBX_PASSWORD", "DBX_WEB_PASSWORD"))
        .is_some()
    {
...
mod tests {
    use super::{
        first_config_value, normalize_public_base_path, web_agent_dir_from_env, web_compression_predicate,
        XLSX_CONTENT_TYPE,
    };
    use axum::body::Body;
    ...
    #[test]
    #[should_panic(expected = "PANDB_PUBLIC_BASE_PATH contains invalid characters")]
    ...
    #[test]
    fn pandb_environment_value_wins_over_legacy_dbx_value() {
        assert_eq!(first_config_value(Some("pandb".to_string()), Some("dbx".to_string())), Some("pandb".to_string()));
        assert_eq!(first_config_value(None, Some("dbx".to_string())), Some("dbx".to_string()));
    }
}
