...
mod tests {
    ...
    fn historical_sqlserver_legacy_setting_is_canonicalized_to_driver_profile() {
        ...
        config.url_params =
            Some("applicationName={PanDB; Client};password=50%;sqlserverEncryption=disabled;encrypt=false".to_string());
        ...
        assert_eq!(canonical.url_params.as_deref(), Some("applicationName={PanDB; Client};password=50%;encrypt=false"));
    ...
    fn sqlserver_legacy_migration_respects_escaped_braces() {
        let params = "applicationName={PanDB}}; Client};sqlserverEncryption=disabled;encrypt=false";

        assert!(sqlserver_legacy_compatibility_param(Some(params)));
        assert_eq!(
            without_sqlserver_legacy_compatibility_param(Some(params)).as_deref(),
            Some("applicationName={PanDB}}; Client};encrypt=false")
        );
    }
