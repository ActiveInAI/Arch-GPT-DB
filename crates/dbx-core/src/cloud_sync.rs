...
const DEFAULT_REMOTE_PATH: &str = "PanDB/sync/snapshot.json";
    ...
    pub async fn put_snapshot(
        ...
        let legacy_snapshot = if let Some(id) = existing_id {
            ...
            if !is_encrypted_snippet_snapshot(&existing_content) {
                ...
                // encrypted PanDB envelope. It must first prove to be a legacy PanDB
                // snapshot, and the caller must explicitly request migration.
                if !is_legacy_dbx_snapshot(&existing_content) {
                    return Err("The selected snippet is not a PanDB sync snapshot; refusing to replace or delete it."
                        .to_string());
                }
                if !self.config.replace_legacy_snippet {
                    return Err(
                        "This snippet contains a legacy unencrypted PanDB snapshot. Use the secure migration action to create an encrypted replacement and delete the legacy snippet only after the new one is created."
                            .to_string(),
                    );
                }
        ...
        let response = match self.config.provider {
            SnippetProvider::GitHub => {
                let payload = serde_json::json!({
                    "description": "PanDB encrypted configuration sync",
                    "public": false,
    ...
    fn request(&self, method: Method, url: &str) -> Result<reqwest::RequestBuilder, String> {
        ...
        Ok(match self.config.provider {
            SnippetProvider::GitHub => request
                ...
                .header(header::USER_AGENT, "PanDB")
...
/// current `SyncSnapshot`: older PanDB releases may not contain fields added
/// since their snapshot was written. At the same time, require the stable PanDB
...
fn parse_legacy_dbx_snapshot(content: &str) -> Result<SyncSnapshot, String> {
    if !is_legacy_dbx_snapshot(content) {
        return Err("The selected snippet is not a PanDB sync snapshot; refusing to replace or delete it.".to_string());
    }
    serde_json::from_str(content).map_err(|_| {
        "The legacy PanDB snapshot is incompatible with this version, so it will not be replaced or deleted."
            .to_string()
    })
}
...
fn gitee_snippet_payload(content: String) -> serde_json::Value {
    serde_json::json!({
        "description": "PanDB configuration sync",
        "public": false,
        "files": { DEFAULT_SNIPPET_FILE_NAME: { "content": content } }
    })
}
...
mod tests {
    ...
    fn normalizes_empty_remote_path_to_default() {
        assert_eq!(normalized_remote_path(None), "PanDB/sync/snapshot.json");
        assert_eq!(normalized_remote_path(Some("")), "PanDB/sync/snapshot.json");
        assert_eq!(normalized_remote_path(Some("///\\\\//")), "PanDB/sync/snapshot.json");
    }
    ...
    fn normalizes_remote_path_separators() {
        ...
        assert_eq!(normalized_remote_path(Some(r"\PanDB\sync\snapshot.json")), "PanDB/sync/snapshot.json");
        assert_eq!(normalized_remote_path(Some("///PanDB//sync/./snapshot.json")), "PanDB/sync/snapshot.json");
        assert_eq!(normalized_remote_path(Some("PanDB/sync/../snapshot.json")), "PanDB/snapshot.json");
    }
    ...
    fn returns_parent_collection_paths_from_leaf() {
        ...
        assert_eq!(
            parent_collection_paths(&normalized_remote_path(Some(r"\PanDB\sync\snapshot.json"))),
            vec!["PanDB".to_string(), "PanDB/sync".to_string()]
        );
    }
    ...
    async fn legacy_snippet_migration_preserves_remote_snapshot() {
        ...
        // This field did not exist in snapshots written by older PanDB versions.
