...
fn oracle_object_statistics_user_segments_sql(schema: &str) -> String {
    // USER_SEGMENTS exposes objects owned by the login/current user, while Arch-GPT DB
...
fn ensure_display_ddl_terminated(sql: String) -> String {
    ...
    // SHOW CREATE TABLE returns a table definition, not a runnable script; Arch-GPT DB
