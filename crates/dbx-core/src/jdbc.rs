...
const JDBC_PLUGIN_DOWNLOAD_URL: &str =
    "https://github.com/ActiveInAI/Arch-GPT-DB/releases/latest/download/dbx-jdbc-plugin-latest.zip";
...
fn install_jdbc_plugin_zip(bytes: &[u8], plugin_dir: &Path) -> Result<(), String> {
    ...
    if manifest.protocol_version != SUPPORTED_PLUGIN_PROTOCOL_VERSION {
        let _ = std::fs::remove_dir_all(&temp_dir);
        return Err(format!(
            "Downloaded JDBC plugin uses protocol version {}, but this Arch-GPT DB build supports protocol version {}",
