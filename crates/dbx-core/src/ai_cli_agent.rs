...
pub fn build_cli_agent_prompt(
    ...
    let database_access = if allow_write_sql {
        "The user explicitly confirmed the proposed database change. Arch-GPT DB MCP tools may execute write and DDL SQL for this run only."
    } else {
        "Use the Arch-GPT DB MCP tools when you need live database schema or read-only query results."
    };
    let mut sections = vec![
        format!("You are running inside Arch-GPT DB Desktop as the {provider_label} CLI provider."),
        database_access.to_string(),
        "Do not modify files or run shell commands. The Arch-GPT DB MCP server is the only intended tool surface."
            .to_string(),
        String::new(),
