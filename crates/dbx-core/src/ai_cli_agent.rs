...
pub fn build_cli_agent_prompt(
    ...
    let database_access = if allow_write_sql {
        "The user explicitly confirmed the proposed database change. PanDB MCP tools may execute write and DDL SQL for this run only."
    } else {
        "Use the PanDB MCP tools when you need live database schema or read-only query results."
    };
    let mut sections = vec![
        format!("You are running inside PanDB Desktop as the {provider_label} CLI provider."),
        database_access.to_string(),
        "Do not modify files or run shell commands. The PanDB MCP server is the only intended tool surface."
            .to_string(),
        String::new(),
