...
pub fn codex_cli_env(config: &AiConfig) -> Result<Vec<(String, String)>, String> {
    ...
    for (key, value) in &config.codex_cli_env {
        ...
        if is_reserved_dbx_mcp_env_name(key) {
            return Err(format!(
                "[codexEnvReserved] `{key}` is managed by PanDB for the scoped MCP server and cannot be set here."
            ));
        }
...
fn classify_codex_spawn_error(message: &str) -> String {
    if message.contains("No such file") || message.contains("not found") {
        "[codexNotInstalled] Codex CLI was not found. Install Codex CLI or set the Codex CLI path in PanDB AI settings."
            .to_string()
    } else if is_command_line_too_long_error(message) {
        "[codexCommandLineTooLong] Codex CLI command line is too long. Update PanDB so Codex prompts are sent through stdin instead of process arguments."
            .to_string()
    } else {
...
fn classify_codex_run_error(stderr: &str) -> String {
    ...
    if lower.contains("not authenticated") || lower.contains("login") || lower.contains("auth") {
        ...
        format!("[dbxMcpMissing] PanDB MCP server was not found. Install @dbx-app/mcp-server and try again. {stderr}")
    } else if lower.contains("mcp") && (lower.contains("dbx") || lower.contains("server")) {
        format!("[codexMcpStartupFailed] Codex could not start the PanDB MCP server. {stderr}")
    } else {
