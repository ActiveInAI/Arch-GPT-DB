...
// Some very old SQL Server setups only accepted Arch-GPT DB <= 0.5.48 because the fallback
                ...
                .map_err(|plain_error| {
                    if is_sqlserver_tls_handshake_error(&encrypted_error) {
                        ...
                        "{encrypted_error}\n\nThis may be caused by an old SQL Server TLS/encryption configuration. \
                         ...
                         when explicitly enabled, Arch-GPT DB can also fall back to the SQL Server legacy compatibility \
