...
fn ensure_mcp_connection_change_allowed_in_tx(
    ...
    if policy.read_only {
        return Err(
            "MCP_READ_ONLY: PanDB global MCP read-only mode is enabled. Connection changes are blocked.".to_string()
        );
    }
    if let Some(connection_id) = target_connection_id {
        if policy.allowed_connection_ids.as_ref().is_some_and(|ids| !ids.iter().any(|id| id == connection_id)) {
            return Err(format!(
                "CONNECTION_OUT_OF_SCOPE: connection '{connection_id}' is not allowed by the current PanDB MCP policy"
            ));
        }
    }
    Ok(())
...
mod tests {
    ...
    async fn mcp_connection_mutations_are_atomic_and_recheck_policy() {
        ...
        // Non-MCP callers remain governed by the ordinary PanDB UI permissions.
