...
async fn run_with_backend(backend: &dyn DbxBackend, flags: Flags) -> Result<String, CliError> {
    ...
    if args.first().is_some_and(|arg| arg == "open") {
        ...
        return Ok(format!("Opened {table} in Arch-GPT DB\n"));
    }
...
async fn run_query(backend: &dyn DbxBackend, flags: &Flags) -> Result<String, CliError> {
    let args = &flags.args;
    let default_connection = env_value("PANDB_CONNECTION", "DBX_CONNECTION");
    ...
    let connection = find_connection(backend, connection_name).await?;
    let env_allow_writes = env_flag("PANDB_MCP_ALLOW_WRITES", "DBX_MCP_ALLOW_WRITES");
    let env_allow_dangerous = env_flag("PANDB_MCP_ALLOW_DANGEROUS_SQL", "DBX_MCP_ALLOW_DANGEROUS_SQL");
    ...
    if connection.db_type == DatabaseType::Redis {
        return Err(CliError::new(
            ...
            "Redis connections do not accept SQL through dbx query. Use an MCP Redis command tool or Arch-GPT DB directly.",
        ));
    }
...
fn env_flag(primary: &str, legacy: &str) -> bool {
    env_value(primary, legacy).is_some_and(|value| matches!(value.trim().to_ascii_lowercase().as_str(), "1" | "true"))
}
    ...
    match format {
        ...
        OutputFormat::Table => {
            let rows = vec![
                ...
                json!({ "mode": "Requires Arch-GPT DB Desktop", "types": BRIDGE_REQUIRED_TYPES.join(", ") }),
            ];
...
async fn diagnostics() -> Diagnostics {
    ...
    let loaded = if db_path_exists {
        ...
        Err("Arch-GPT DB database does not exist.".to_string())
    };
...
fn format_diagnostics(value: &Diagnostics, format: OutputFormat) -> Result<String, CliError> {
    ...
    let rows = vec![
        ...
        json!({ "check": "Arch-GPT DB database", "value": if value.db_path_exists { format!("found ({})", value.db_path) } else { format!("missing ({})", value.db_path) } }),
