...
async fn run_with_backend(backend: &dyn DbxBackend, flags: Flags) -> Result<String, CliError> {
    ...
    if args.first().is_some_and(|arg| arg == "open") {
        ...
        return Ok(format!("Opened {table} in PanDB\n"));
    }
...
async fn run_query(backend: &dyn DbxBackend, flags: &Flags) -> Result<String, CliError> {
    ...
    if connection.db_type == DatabaseType::Redis {
        return Err(CliError::new(
            ...
            "Redis connections do not accept SQL through dbx query. Use an MCP Redis command tool or PanDB directly.",
        ));
    }
    ...
    match format {
        ...
        OutputFormat::Table => {
            let rows = vec![
                ...
                json!({ "mode": "Requires PanDB Desktop", "types": BRIDGE_REQUIRED_TYPES.join(", ") }),
            ];
...
async fn diagnostics() -> Diagnostics {
    ...
    let loaded = if db_path_exists {
        ...
        Err("PanDB database does not exist.".to_string())
    };
...
fn format_diagnostics(value: &Diagnostics, format: OutputFormat) -> Result<String, CliError> {
    ...
    let rows = vec![
        ...
        json!({ "check": "PanDB database", "value": if value.db_path_exists { format!("found ({})", value.db_path) } else { format!("missing ({})", value.db_path) } }),
