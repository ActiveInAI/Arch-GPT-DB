...
pub trait DbxBackend: Send + Sync {
    ...
    async fn bridge_request(&self, path: &str, body: Value) -> Result<(), String> {
        let _ = (path, body);
        Err("PanDB is not running. Please start PanDB first.".to_string())
    }
...
impl WebBackend {
    ...
    async fn ensure_auth(&self) -> Result<(), String> {
        ...
        if check.setup_required {
            return Err("PanDB Web password setup is required before MCP Web mode can access APIs.".to_string());
        }
        ...
        if self.password.is_empty() {
            return Err("PanDB Web authentication is required. Set DBX_WEB_PASSWORD for MCP Web mode.".to_string());
        }
        let response = self
            .client
        ...
        let cookie = response
            ...
            .ok_or_else(|| "Authentication failed: PanDB Web did not return a session cookie.".to_string())?;
...
impl LocalBackend {
    ...
    /// shares the PanDB process so it is unaffected by this cache desync.
...
impl DbxBackend for LocalBackend {
    ...
    async fn load_connections(&self) -> Result<Vec<ConnectionConfig>, String> {
        ...
        // Connections created/modified/deleted in the PanDB desktop UI after this process started
    ...
    async fn bridge_request(&self, path: &str, body: Value) -> Result<(), String> {
        let port = tokio::fs::read_to_string(self.data_dir.join("mcp-bridge-port"))
            .await
            .map_err(|_| "PanDB is not running. Please start PanDB first.".to_string())?;
        ...
        if response.status().is_success() {
            ...
            Err(response.text().await.unwrap_or_else(|_| "PanDB bridge request failed.".to_string()))
        }
    }
}
...
impl DbxBackend for WebBackend {
    ...
    async fn execute_agent_tool(
        ...
        let result = async {
            if tool_name != "execute_query" {
                return Err(format!("Unsupported PanDB Web agent tool: {tool_name}"));
            }
            if connection.db_type == DatabaseType::MongoDb {
                return Err(
                    "MongoDB shell commands in PanDB Web mode are not implemented by the Rust MCP yet.".to_string()
                );
            }
    ...
    async fn execute_query(
        ...
        if connection.db_type == DatabaseType::MongoDb {
            return Err("MongoDB shell commands in PanDB Web mode are not implemented by the Rust CLI yet.".to_string());
        }
        self.ensure_connected(connection).await?;
        self.request(
            reqwest::Method::POST,
...
mod tests {
    ...
    fn local_plugin_directory_honors_desktop_storage_settings() {
        ...
        let explicit = DesktopSettings {
            plugin_store_dir: Some("D:/PanDB/plugins-custom".to_string()),
            ..DesktopSettings::default()
        };
        let legacy =
            DesktopSettings { driver_store_dir: Some("D:/PanDB/drivers".to_string()), ..DesktopSettings::default() };

        assert_eq!(local_plugin_dir(&explicit, data_dir), PathBuf::from("D:/PanDB/plugins-custom"));
        assert_eq!(local_plugin_dir(&legacy, data_dir), PathBuf::from("D:/PanDB/drivers/plugins"));
    }
