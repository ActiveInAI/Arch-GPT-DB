...
pub struct ConnectionSelector {
    #[schemars(description = "Unique ID of the PanDB connection")]
    pub connection_id: Option<String>,
    #[schemars(description = "Name of the PanDB connection")]
...
impl DbxMcpServer {
    #[tool(
        ...
        description = "List database connections configured in PanDB. Returns connection IDs, names, database types, endpoints, and selected databases."
    )]
    ...
    async fn execute_redis_command(
        ...
        if safety != RedisCommandSafety::Allowed && resolved.policy.read_only {
            return tool_error("MCP_READ_ONLY", "PanDB global MCP read-only mode is enabled. Redis command blocked.");
        }
        ...
        if safety == RedisCommandSafety::Blocked && !permissions.allow_dangerous {
            return tool_error(
                ...
                format!(
                    "Dangerous Redis command \"{}\" is disabled in PanDB MCP settings.",
        ...
        if safety != RedisCommandSafety::Allowed && !permissions.allow_writes {
            return tool_error(
                "REDIS_COMMAND_BLOCKED",
                "MCP Redis command execution is read-only in PanDB MCP settings.",
            );
        }
    ...
    #[tool(name = "dbx_add_connection", description = "Add a new database connection to PanDB")]
    async fn add_connection(&self, Parameters(request): Parameters<AddConnectionRequest>) -> CallToolResult {
        ...
        if policy.read_only {
            return tool_error(
                "MCP_READ_ONLY",
                "PanDB global MCP read-only mode is enabled. Connection management is not allowed.",
            );
        }
    ...
    #[tool(name = "dbx_remove_connection", description = "Remove a database connection from PanDB")]
    ...
    #[tool(name = "dbx_open_table", description = "Open a table in PanDB desktop app. Requires PanDB to be running.")]
    async fn open_table(&self, Parameters(request): Parameters<OpenTableRequest>) -> CallToolResult {
        ...
        match self
            ...
            .await
        {
            Ok(()) => text(format!("Opened {} in PanDB", request.table)),
    ...
    #[tool(
        ...
        description = "Execute a SQL query in PanDB desktop app UI and show results there. Requires PanDB to be running."
    )]
    ...
    async fn resolve_connection(&self, selector: &ConnectionSelector) -> Result<ResolvedConnection, CallToolResult> {
        ...
        if let Some(id) = selector.connection_id.as_deref().map(str::trim).filter(|id| !id.is_empty()) {
            ...
            if self.scope.connection_scope_enabled() && !self.scope.matches(&connection) {
                return Err(tool_error(
                    ...
                    format!("Connection \"{id}\" is outside this PanDB AI session scope."),
                ));
            }
            if !policy_allows_connection(&policy, &connection) {
                return Err(tool_error(
                    "CONNECTION_OUT_OF_SCOPE",
                    format!("Connection \"{id}\" is not allowed by PanDB MCP settings."),
                ));
            }
        ...
        if self.scope.connection_scope_enabled() {
            let connection = connections
                ...
                .ok_or_else(|| tool_error("CONNECTION_NOT_FOUND", "Scoped PanDB connection was not found."))?;
            if let Some(name) = selector.connection_name.as_deref().map(str::trim).filter(|name| !name.is_empty()) {
                if name != connection.name && name != connection.id {
                    return Err(tool_error(
                        "CONNECTION_OUT_OF_SCOPE",
                        format!("Connection \"{name}\" is outside this PanDB AI session scope."),
                    ));
                }
            }
            if !policy_allows_connection(&policy, &connection) {
                return Err(tool_error(
                    "CONNECTION_OUT_OF_SCOPE",
                    "The PanDB AI session scope is outside the global MCP connection allowlist.",
                ));
            }
            return Ok(ResolvedConnection { connection, policy });
        }
        ...
        match allowed.as_slice() {
            ...
            [] => Err(tool_error(
                "CONNECTION_OUT_OF_SCOPE",
                format!("Connection \"{name}\" is not allowed by PanDB MCP settings."),
            )),
...
impl ServerHandler for DbxMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions("Use PanDB connections to inspect schemas and query databases safely.")
    }
}
...
fn validate_sql_policy(
    ...
    if policy.read_only && is_write {
        return Err(tool_error("MCP_READ_ONLY", "PanDB global MCP read-only mode is enabled. SQL write blocked."));
    }
    ...
    if high_risk && !policy.allow_dangerous_sql && confirmed_sql.is_none() {
        return Err(tool_error("SQL_BLOCKED", "High-risk SQL is disabled in PanDB MCP settings."));
    }
        ...
        mongo::validate_safety(&command, permissions.allow_writes, permissions.allow_dangerous, production_database)
    {
        return Err(match error {
            MongoSafetyError::WritesDisabled => tool_error(
                ...
                "MCP MongoDB execution is read-only in PanDB MCP settings.",
            ),
            MongoSafetyError::EmptyFilter => tool_error(
                "SQL_BLOCKED",
                "MongoDB update/delete commands must include a non-empty filter unless high-risk operations are enabled in PanDB MCP settings.",
            ),
            MongoSafetyError::Dangerous => tool_error(
                "SQL_BLOCKED",
                "Dangerous MongoDB command is disabled in PanDB MCP settings.",
            ),
