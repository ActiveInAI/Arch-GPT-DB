...
The contributors page treats an account as eligible only when it has authored at least one pull request merged into `ActiveInAI/Arch-GPT DB`. Commit counts are displayed as an additional metric but do not grant eligibility by themselves.
