...
**Goal:** Build the dbdocs-style documentation viewer as a standalone Vue app that renders a `SchemaSnapshot` and makes **zero backend calls** — the contract that later lets the same components run inside PanDB and inside an exported HTML file.
...
describe("docs viewer component contract", () => {
  ...
  it("makes no backend calls", () => {
    ...
    // exported HTML file on a machine that has never installed PanDB.
    const forbidden = [
      "@/lib/backend",
      "@tauri-apps",
      "invoke(",
...
Style with Tailwind classes and PanDB's existing token names (`bg-background`, `text-foreground`, `border-border`, `text-muted-foreground`). Group colours come only from `var(--group-c)` / `var(--group-tint)` driven by the `--h` custom property.
...
- Mounting the viewer inside PanDB (`DatabaseDocsDialog.vue`) and the editing affordances behind `readonly=false`.
