...
DBML's `TableGroup [color: #rrggbb]` needs hex, but groups store a hue. Convert at the theme-neutral lightness `L = 0.55`, `C = 0.15` — the light-theme `--group-c` from the spec — so an exported DBML file opened in dbdocs looks like PanDB's light theme.
...
With a real connection configured in PanDB:
