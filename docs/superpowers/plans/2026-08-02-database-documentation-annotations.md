...
6. **Annotations are applied CLI-side, NOT inside the collector.** The notes file lives on the
   machine running `dbx`, and `WebBackend` talks to a remote PanDB instance that cannot read it.
...
- Writing the notes file from PanDB (Part 2 only reads it; the file is hand-edited or repo-managed).
