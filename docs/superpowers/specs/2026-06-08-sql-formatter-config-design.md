...
- `formatSqlText(sql, dialect)` maps the PanDB dialect to the formatter language.
...
7. `formatSqlText()` maps PanDB settings to `sql-formatter` options and returns formatted SQL.
