...
Give ClickHouse connections a dedicated SQL highlighting dialect in PanDB's
...
PanDB currently has separate dialect-selection paths for SQL formatting and SQL
highlighting:
...
- CodeMirror-based DDL and SQL viewers that use the shared PanDB dialect factory
...
Add `clickhouse` as a first-class PanDB CodeMirror dialect name. Define its
...
do not share all lexical behavior. In particular, PanDB already tests that
