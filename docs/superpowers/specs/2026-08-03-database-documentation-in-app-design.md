# Database documentation in PanDB — design
...
This part makes the viewer reachable and editable inside PanDB. The standalone HTML export, the
...
**Explicitly not built**: a second ER diagram. PanDB already ships `SchemaDiagramDialog.vue`
...
file that runs on a machine with no PanDB installed.
