...
PanDB can inspect a schema but cannot document it. There is no way to produce a browsable, shareable description of a database, and no occurrence of DBML anywhere in the repository.
...
The documentation UI is a **standalone Vue application that makes no backend calls**. The same component tree renders inside PanDB as a dialog and inside an exported single-file HTML page. Generation is available headless through `dbx docs` and `dbx dbml`, so documentation can be regenerated in CI and schema drift can fail a build.
...
- Collect a normalized `SchemaSnapshot` for any relational engine PanDB already supports, reusing the existing `schema::providers` metadata layer.
...
- Let users author table notes, column notes, project notes, and **table groups** in PanDB, persisted in a human-readable, git-committable file.
...
- **Write-back to the database.** PanDB never issues `COMMENT ON` as a side effect of documentation.
- **Publishing/hosting.** PanDB writes a file; distributing it is the user's business.
...
- `apps/desktop/src/lib/diagram/erDiagram.ts` — `layoutDiagramTables` and `diagramZoom.ts`. This module imports **only types**, so the standalone docs bundle can import it without pulling in PanDB runtime code. `buildDiagramRelationships` / `foreignKeySourceCardinality` are the reference implementation for the Rust port in Section 3.
- `apps/desktop/src/lib/export/diagramSvgExport.ts` — existing SVG serialization for diagram export.
- `apps/desktop/src/styles/globals.css:1310+` — the PanDB token palette (strictly achromatic, `oklch(… 0 0)`), from which the docs theme subset is derived.
...
- **`format_version`** guards viewer/data skew. The exported HTML pins a snapshot beside a viewer built from the same commit, but an in-app viewer may load a snapshot produced by an older PanDB. One integer prevents a class of blank-render bugs and makes future additions additive.
...
Project ecommerce {
  ...
  Note: '''Generated by PanDB on 2026-08-02'''
}
...
**The contract: this directory makes no backend calls.** No `api.*`, no Tauri, no Pinia stores. Snapshot in, DOM out. This is what allows the same code to run inside an HTML file on a machine that has never installed PanDB, and what makes the entire documentation UI testable without a database.
...
- **Theme tokens are a curated subset**, not `globals.css` (2362 lines). The viewer has its own Tailwind v4 entry scanning only `src/docs/**` plus roughly 25 hand-picked tokens matching PanDB's variable names, so it inherits live tokens in-app and ships its own values (with `prefers-color-scheme` and a manual toggle) when exported.
...
5. `DatabaseDocsDialog.vue` — wire into PanDB.
