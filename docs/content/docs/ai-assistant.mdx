...
Arch-GPT DB includes presets for:
...
Compatible and custom configurations can set the endpoint, model, API style, authentication method, proxy, and context window. API styles include OpenAI-style completions, the Responses API, and Anthropic Messages. When supported, Arch-GPT DB discovers models and filters out endpoints that are not suitable for assistant or Agent use.
...
You can configure the executable path and additional environment variables. CLI Agents run on the Desktop computer and access the active database through Arch-GPT DB-scoped MCP tools. Docker/Web hides these local CLI providers because the server cannot assume the browser computer has the commands installed.
...
Multiple templates can be active at once. Arch-GPT DB snapshots the selected templates and global instructions when the request is sent, so later edits do not alter an already running task.
...
5. AI Agents cannot receive write or DDL authorization for production databases; they can only return SQL for manual review in Arch-GPT DB
...
Providers expose different mechanisms: enumerated effort, reasoning levels, boolean thinking, integer budgets, or free-text options. Arch-GPT DB shows controls from the active model capability and remembers the selection per configuration and model.
...
<Cards>
  ...
  <Card title="MCP Integration" href="/en/docs/mcp">
    Connect external AI coding assistants to databases through Arch-GPT DB.
  </Card>
