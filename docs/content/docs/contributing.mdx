...
description: Set up PanDB from scratch, run the desktop app, make your first change, test it, and open a pull request.
---

This tutorial is for first-time PanDB contributors. By the end, you will have:

1. Installed Node.js, pnpm, Rust, and native build dependencies
2. Forked, cloned, and run PanDB Desktop
...
Open [PanDB Issues](https://github.com/ActiveInAI/PanDB/issues) and choose an issue that has no assignee and no active contributor in its comments. Good first contributions have one of these properties:
...
PanDB Desktop uses Tauri, Vue, and Rust. The repository currently requires:
...
See the [official Tauri prerequisites](https://v2.tauri.app/start/prerequisites/) for complete platform details. PanDB also uses ODBC development libraries.
...
Open [ActiveInAI/PanDB](https://github.com/ActiveInAI/PanDB) on GitHub and click **Fork**.
...
cd dbx
git remote add upstream https://github.com/ActiveInAI/PanDB.git
git remote -v
```
...
## 4. Run PanDB for the First Time
...
Development builds can run alongside an installed PanDB instance. Both use the same local PanDB data, so connections and history are available in each window. Avoid changing the same connection, saved SQL, or global setting in both windows at once.
...
A successful run opens the PanDB desktop window without a compiler error in the terminal. Complete three quick checks:

1. Open Settings
2. Create or open a local test connection
3. Close and start the app again to confirm the setup is repeatable

If port `1420` is already in use, stop the previous PanDB/Vite development process and restart.
...
Agents are separate processes that communicate with PanDB through stdin/stdout JSON-RPC. Java/JDBC agents normally use JDK 21; native agents use Go or Rust. Read these first:

- [`agents/README.md`](https://github.com/ActiveInAI/PanDB/blob/main/agents/README.md)
- [`agents/docs/agent-authoring.md`](https://github.com/ActiveInAI/PanDB/blob/main/agents/docs/agent-authoring.md)
...
A successful build does not mean the local PanDB app is using the new code. PanDB runs `.dbx/agents/drivers/<db_type>/agent.jar` under the user home directory, so back up and replace the runtime JAR:
...
Restart PanDB or disconnect and reconnect the database so the old agent process exits and the new JAR is loaded. Repeat the issue's reproduction flow against the real database version.
...
Open your fork on GitHub and click **Compare & pull request**. Set the target repository to `ActiveInAI/PanDB` and the target branch to `main`.
...
- Read the repository [CONTRIBUTING.md](https://github.com/ActiveInAI/PanDB/blob/main/CONTRIBUTING.md)
