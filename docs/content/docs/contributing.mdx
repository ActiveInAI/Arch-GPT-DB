...
description: Set up Arch-GPT DB from scratch, run the desktop app, make your first change, test it, and open a pull request.
---

This tutorial is for first-time Arch-GPT DB contributors. By the end, you will have:

1. Installed Node.js, pnpm, Rust, and native build dependencies
2. Forked, cloned, and run Arch-GPT DB Desktop
...
Open [Arch-GPT DB Issues](https://github.com/ActiveInAI/Arch-GPT DB/issues) and choose an issue that has no assignee and no active contributor in its comments. Good first contributions have one of these properties:
...
Arch-GPT DB Desktop uses Tauri, Vue, and Rust. The repository currently requires:
...
See the [official Tauri prerequisites](https://v2.tauri.app/start/prerequisites/) for complete platform details. Arch-GPT DB also uses ODBC development libraries.
...
Open [ActiveInAI/Arch-GPT DB](https://github.com/ActiveInAI/Arch-GPT DB) on GitHub and click **Fork**.
...
cd dbx
git remote add upstream https://github.com/ActiveInAI/Arch-GPT DB.git
git remote -v
```
...
## 4. Run Arch-GPT DB for the First Time
...
Development builds can run alongside an installed Arch-GPT DB instance. Both use the same local Arch-GPT DB data, so connections and history are available in each window. Avoid changing the same connection, saved SQL, or global setting in both windows at once.
...
A successful run opens the Arch-GPT DB desktop window without a compiler error in the terminal. Complete three quick checks:

1. Open Settings
2. Create or open a local test connection
3. Close and start the app again to confirm the setup is repeatable

If port `1420` is already in use, stop the previous Arch-GPT DB/Vite development process and restart.
...
Agents are separate processes that communicate with Arch-GPT DB through stdin/stdout JSON-RPC. Java/JDBC agents normally use JDK 21; native agents use Go or Rust. Read these first:

- [`agents/README.md`](https://github.com/ActiveInAI/Arch-GPT DB/blob/main/agents/README.md)
- [`agents/docs/agent-authoring.md`](https://github.com/ActiveInAI/Arch-GPT DB/blob/main/agents/docs/agent-authoring.md)
...
A successful build does not mean the local Arch-GPT DB app is using the new code. Arch-GPT DB runs `.dbx/agents/drivers/<db_type>/agent.jar` under the user home directory, so back up and replace the runtime JAR:
...
Restart Arch-GPT DB or disconnect and reconnect the database so the old agent process exits and the new JAR is loaded. Repeat the issue's reproduction flow against the real database version.
...
Open your fork on GitHub and click **Compare & pull request**. Set the target repository to `ActiveInAI/Arch-GPT DB` and the target branch to `main`.
...
- Read the repository [CONTRIBUTING.md](https://github.com/ActiveInAI/Arch-GPT DB/blob/main/CONTRIBUTING.md)
