...
Field lineage helps answer where a column comes from, where it is used, and what a change might affect. Arch-GPT DB combines database metadata with local query history and labels evidence strength instead of treating every matching name as a confirmed dependency.
...
Arch-GPT DB uses executed queries as supporting evidence, including:
...
History only represents SQL executed or saved in Arch-GPT DB. It does not cover all application code, ETL, BI, or other clients.
...
- Without declared foreign keys, Arch-GPT DB can only provide view, history, and same-name clues
