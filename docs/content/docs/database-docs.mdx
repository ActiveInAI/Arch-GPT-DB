...
Database documentation turns a live schema into something readable. PanDB collects tables, columns, indexes, and relationships into a snapshot, merges it with notes you write yourself, and renders a browsable reference. The same snapshot serializes to DBML, so the diagram tools and CI checks read exactly what the viewer shows.
...
A column that has a database `COMMENT ON` value and a note written in PanDB shows your note marked **LOCAL**, with the database comment preserved underneath. Nothing is overwritten in the database — PanDB never issues DDL for documentation, and the original comment is always recoverable.

## Where Notes Are Stored

By default each connection gets its own notes file inside the PanDB data directory, so the feature works with no setup.
...
| Not representable in DBML | A construct is documented in PanDB but cannot be expressed in exported DBML. |
...
- DBML export is one-way. A `.dbml` file is an output, never a source of truth — PanDB does not import it.
