...
<Callout type="info">Arch-GPT DB can open connection layers for databases that are not directly reachable. Configure them from the connection dialog's **Tunnel / Proxy** tab.</Callout>

<Callout type="warning">
  Tunnels run where the Arch-GPT DB backend runs. On Desktop, local paths and temporary listeners belong to your computer. In Docker/Web, paths, network egress, and temporary listeners belong to the server or container, not the computer running the browser.
</Callout>

Arch-GPT DB supports three tunnel / proxy types:
...
For network databases, Arch-GPT DB opens a temporary port in its current runtime and points the database driver at that port. The tunnel / proxy layer forwards protocol bytes to the real target. A connection may combine multiple transport layers, which Arch-GPT DB establishes in the order shown in the connection dialog.
...
| Connect Timeout | Maximum time Arch-GPT DB waits while opening the SSH connection; default is 5 seconds |
...
<Tabs groupId="ssh-auth" items={['Private Key (Recommended)', 'Private Key + Password', 'Password', 'No Credential']}>
  <Tab value="Private Key (Recommended)">
    Enter a private key file that the Arch-GPT DB backend can read, for example `~/.ssh/id_rsa` or `~/.ssh/id_ed25519`:
  ...
  <Tab value="Private Key + Password">
    Use a private key as the primary credential with password as a fallback. Arch-GPT DB tries key authentication first; if the server rejects the key and continues to offer password authentication, it automatically falls back to password authentication.
...
Arch-GPT DB verifies the SSH host key before it sends a credential. Trusted keys come from the system `known_hosts` files and Arch-GPT DB's own `known_hosts` under the data directory:

- An unknown host prompts you to review and explicitly accept its fingerprint.
- You can trust it for the current connection only or remember it in Arch-GPT DB's `known_hosts`.
- Arch-GPT DB rejects a changed key for a previously known host.
...
<Callout type="warn">LAN exposure changes the SSH listener from `127.0.0.1` to `0.0.0.0`. This expands the attack surface of the Arch-GPT DB runtime and should only be used on trusted networks with appropriate firewall rules.</Callout>
...
<Callout type="info">The feature is an HTTP tunnel. The script currently provided by Arch-GPT DB is `dbx_tunnel.php`, so the deployment artifact is PHP-specific, but the connection type in Arch-GPT DB is HTTP tunnel.</Callout>
...
| -------------------------------- | -------- | --------------------------------------------------------------------------- |
| `DBX_TUNNEL_TOKEN`               | Yes      | Shared token. Enter the same value in Arch-GPT DB.                                  |
...
In Arch-GPT DB, add an **HTTP Tunnel** layer and fill:
...
```text
Arch-GPT DB database driver
  -> 127.0.0.1:<local temporary port>
  -> Arch-GPT DB HTTP tunnel client
  -> HTTP POST requests
...
1. Arch-GPT DB starts a TCP listener in its current runtime.
2. The database driver connects to that local port.
3. Arch-GPT DB sends `open` to `dbx_tunnel.php` with a session id, target host, target port, and connect timeout.
...
6. Arch-GPT DB polls the script with `read`; response bytes are written back to the local database driver connection.
7. When the connection ends, Arch-GPT DB sends `close` and the PHP worker closes the database socket.

This does not require Arch-GPT DB Web and does not use WebSocket.
...
An Oracle TNS descriptor may contain multiple failover addresses. Arch-GPT DB does not rewrite that descriptor through one temporary endpoint, so TNS mode cannot be combined with SSH, proxy, or HTTP tunnel layers. Use Service Name or SID mode when a tunnel is required.
