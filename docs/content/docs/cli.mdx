---
title: PanDB CLI
description: Use the native PanDB CLI to read connections, schemas, query results, and AI context from terminals, scripts, CI, and Codex.
---

<Callout type="info">PanDB CLI is a dedicated command line package for terminal, script, and coding-agent workflows. It shares PanDB connection storage and SQL safety rules with the MCP server.</Callout>
...
Set `DBX_DATA_DIR` when using a custom or portable PanDB data directory.
...
Use `dbx doctor` to inspect local PanDB paths, connection-store health, and whether the desktop bridge is available:
...
Use `dbx capabilities` to see which database types can be queried directly and which currently require PanDB Desktop:
...
The CLI binary currently lists PostgreSQL, Redshift, MySQL, Doris, StarRocks, Manticore Search, SQLite, rqlite, KWDB, and QuestDB as direct-query types. `dbx capabilities` is authoritative for the installed version. Other types require a running PanDB Desktop bridge or their Agent/external-driver infrastructure.
...
- Agent/JDBC, DuckDB, and other external-driver paths still require matching components installed through PanDB
- `dbx open` is always a Desktop UI operation and returns `DBX_NOT_RUNNING` when the app is unavailable

### PanDB Web and Docker

Set `DBX_WEB_URL` to use a deployed PanDB Web backend instead of local connection storage:
...
`dbx query` executes SQL for SQL connections and accepts supported Mongo shell commands for MongoDB connections. Redis is not executed through this command; use the MCP Redis tool or the PanDB workspace instead. Query mode is read-only by default.
...
Dangerous SQL such as `DROP`, `TRUNCATE`, and `ALTER` requires both `--allow-writes` and `--allow-dangerous-sql`. Explicit transaction statements remain blocked by CLI; use [MCP stateful sessions](/en/docs/mcp#stateful-query-sessions) or the PanDB editor for pinned multi-step sessions.
...
PanDB Desktop supports `dbx://connection/new` links for opening the new connection dialog from browsers, bastion hosts, or scripts with connection fields prefilled.
...
| `name`       | Display name in PanDB; when omitted, PanDB uses `database`, then `host`                      |
...
<Callout type="info">Before testing system-level `dbx://` launches, install and open PanDB Desktop once so the operating system can register the URL scheme. On macOS, use `open 'dbx://connection/new?...'` to test it.</Callout>
