...
description: Understand PanDB connection profiles, driver runtimes, specialized workspaces, and capability-gated advanced features.
---
...
Protocol compatibility covers only connection and selected dialect behavior. Versions, system catalogs, DDL, privileges, and management APIs differ, so PanDB uses separate profiles and capability overrides instead of enabling features by family name alone.
...
| --- | --- | --- |
| Built-in native | Rust driver compiled into PanDB | Available immediately after installation |
| Native Agent / sidecar | PanDB-managed Go, Rust, or external process | Requires platform files; DuckDB, Oracle, KingBase, XuguDB, RabbitMQ, and similar paths may use separate runtimes |
...
Menu availability is therefore more meaningful than a static database name list. When an action is missing, check the PanDB version, database profile, runtime mode, and installed components.
...
- Cloud Sync and configuration export migrate PanDB settings, not business database contents
...
PanDB parses many common DSNs and URLs, but parameters vary by version and cloud vendor. After a successful connection test, still verify the actual database, schema, SSL, and privilege scope.
