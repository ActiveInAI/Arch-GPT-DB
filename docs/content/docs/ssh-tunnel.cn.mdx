...
<Callout type="info">当数据库不能直接访问时，Arch-GPT DB 可以在连接窗口的 **隧道/代理** 页签里配置连接层。</Callout>

<Callout type="warning">
  隧道实际运行在 Arch-GPT DB 后端所在的位置。桌面版中的“本地路径”和临时监听端口属于当前电脑；Docker/Web 版中的路径、网络出口和临时监听端口属于服务器或容器，不属于浏览器所在电脑。
</Callout>

Arch-GPT DB 支持三类隧道/代理：
...
对网络数据库来说，Arch-GPT DB 会先在当前运行环境中打开一个临时端口，让数据库驱动连接这个端口，再由隧道/代理层把数据库协议字节转发到真实目标。一个连接可以组合多层传输配置，Arch-GPT DB 会按照连接窗口中的顺序建立链路。
...
| 连接超时   | Arch-GPT DB 等待 SSH 连接建立的最长时间，默认 5 秒 |

<Tabs groupId="ssh-auth" items={['密钥认证（推荐）', '密钥+密码', '密码认证', '无凭据']}>
  <Tab value="密钥认证（推荐）">
    填写 Arch-GPT DB 后端可以读取的 SSH 私钥文件，例如 `~/.ssh/id_rsa`、`~/.ssh/id_ed25519`：
  ...
  <Tab value="密钥+密码">
    以私钥为主要认证方式，密码作为回退。Arch-GPT DB 会先尝试密钥认证，如果服务器拒绝了密钥且仍支持密码认证，会自动回退到密码认证。
...
Arch-GPT DB 会在发送凭据前验证 SSH 主机密钥。已知密钥从系统 `known_hosts` 和 Arch-GPT DB 数据目录中的 `known_hosts` 读取：

- 第一次连接到未知主机时，Arch-GPT DB 会显示指纹并要求明确接受；
- 可以只信任本次连接，也可以记住到 Arch-GPT DB 的 `known_hosts`；
- 已记录主机的密钥发生变化时，Arch-GPT DB 会拒绝连接；
- 如果确认窗口不可用、超时或被拒绝，连接会失败，并且不会发送密码或私钥。

### 允许局域网访问

<Callout type="warn">启用“允许局域网访问”会把 SSH 临时监听端口从 `127.0.0.1` 改为 `0.0.0.0`。这会扩大当前 Arch-GPT DB 运行环境的攻击面，只应在可信网络并配合防火墙使用。</Callout>
...
<Callout type="info">功能类型叫 HTTP 隧道。Arch-GPT DB 当前随代码提供的部署脚本是 `dbx_tunnel.php`，所以部署文件是 PHP 实现，但连接类型本身不是“PHP 隧道”。</Callout>
...
| 变量                             | 必填 | 说明                                         |
| -------------------------------- | ---- | -------------------------------------------- |
| `DBX_TUNNEL_TOKEN`               | 是   | 共享 token。Arch-GPT DB 里填写同一个值。             |
...
在 Arch-GPT DB 里添加 **HTTP 隧道** 层，并填写：
...
1. Arch-GPT DB 在当前运行环境中启动一个 TCP 监听端口。
2. 数据库驱动连接这个本地端口。
3. Arch-GPT DB 向 `dbx_tunnel.php` 发送 `open`，带上 session id、目标主机、目标端口和连接超时。
4. PHP 脚本校验 token 和 allow-list，然后启动 worker，worker 打开到数据库的 TCP socket。
5. 数据库驱动发出的字节由 Arch-GPT DB 通过 `write` 发给脚本。
6. Arch-GPT DB 通过 `read` 轮询脚本；脚本返回数据库响应字节后，Arch-GPT DB 写回本地数据库驱动连接。
7. 连接结束时，Arch-GPT DB 发送 `close`，PHP worker 关闭数据库 socket。

这个方案不需要部署 Arch-GPT DB Web，也不使用 WebSocket。
...
Oracle TNS 描述符可能包含多个故障转移地址，Arch-GPT DB 不会把它重写到单个临时端口，因此 TNS 模式不能和 SSH、代理或 HTTP 隧道组合。需要隧道时请改用 Service Name 或 SID 模式。
