---
title: PanDB CLI
description: 使用原生 PanDB CLI 在终端、脚本、CI 和 Codex 中读取连接、Schema、查询结果和 AI 上下文。
---

<Callout type="info">PanDB CLI 是独立命令行包，适合终端、脚本和 AI 编程助手工作流。它与 MCP Server 共享 PanDB 连接存储和 SQL 安全规则。</Callout>
...
如果使用自定义或便携版 PanDB 数据目录，可以设置 `DBX_DATA_DIR`。
...
使用 `dbx doctor` 查看本地 PanDB 路径、连接存储健康状态，以及桌面端 bridge 是否可用：
...
使用 `dbx capabilities` 查看哪些数据库类型可以直接查询，哪些当前需要 PanDB 桌面端：
...
当前原生直连清单由 CLI 二进制维护，包括 PostgreSQL、Redshift、MySQL、Doris、StarRocks、Manticore Search、SQLite、rqlite、KWDB 和 QuestDB。`dbx capabilities` 是当前版本的权威输出；其他类型需要运行中的 PanDB Desktop bridge 或对应 Agent/外部驱动基础设施。
...
- Agent/JDBC、DuckDB 和其它外部驱动路径仍需要在 PanDB 中安装匹配组件
- `dbx open` 始终是 Desktop UI 操作，Desktop 未运行时返回 `DBX_NOT_RUNNING`

### PanDB Web / Docker

设置 `DBX_WEB_URL` 后，CLI 改为使用部署的 PanDB Web 后端，而不是读取本机连接存储：
...
`dbx query` 对 SQL 连接执行一条 SQL，对 MongoDB 连接接受受支持的 Mongo shell 命令。Redis 不通过这个命令执行，需使用 MCP Redis 工具或 PanDB 专用工作区。默认只读。
...
`DROP`、`TRUNCATE`、`ALTER` 等危险 SQL 需要同时显式使用 `--allow-writes` 和 `--allow-dangerous-sql`。显式事务语句仍会被 CLI 阻止；需要多步固定 Session 时使用 [MCP 有状态会话](/cn/docs/mcp#有状态查询会话) 或 PanDB 编辑器。
...
| `CONNECTION_STORE_ERROR` | PanDB 连接存储存在，但无法读取 |
| `CONNECTION_NOT_FOUND`   | 找不到指定的 PanDB 连接        |
| `SQL_BLOCKED`            | SQL 被安全规则拦截           |
| `DBX_NOT_RUNNING`        | PanDB 桌面端 bridge 不可用     |
| `ERROR`                  | 未预期的运行时错误           |

## 桌面端 Deep Link

PanDB 桌面端支持通过 `dbx://connection/new` 从浏览器、堡垒机或脚本打开新建连接窗口并预填连接信息。
...
| `name`       | PanDB 中显示的连接名；未传时优先使用 `database`，其次使用 `host` |
| `host`       | 主机地址                                                       |
| `port`       | 端口                                                           |
| `user`       | 用户名                                                         |
| `password`   | 密码                                                           |
...
<Callout type="info">测试系统级 `dbx://` 唤起前，请先安装并启动一次 PanDB 桌面端，让系统注册 URL 协议。macOS 可使用 `open 'dbx://connection/new?...'` 测试。</Callout>
