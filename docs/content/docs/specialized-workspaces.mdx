...
When a connection is read-only, PanDB disables or rejects mutations in Redis, MongoDB, and GridFS. Dangerous Redis commands also pass through a separate command-safety classifier.

## Search and Document Indexes

Elasticsearch, Easysearch, and Manticore Search use index- and document-oriented browsing. PanDB displays indexes or collections from driver metadata and provides entry points for browsing, querying, and inspecting documents.
...
The products use different APIs, collection models, and search parameters. PanDB generates an engine-specific request template, but dimensions, field names, distance metrics, and index settings must match the target database.

<Callout type="warn">Vector upserts and deletes are write operations. Production connections should combine database privileges, PanDB read-only/production protection, and human request review.</Callout>
...
- Production protection still applies to writes recognized by PanDB, while database privileges remain the final boundary.
