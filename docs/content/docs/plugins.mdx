---
title: JDBC Plugin
description: Install the Arch-GPT DB JDBC plugin, import vendor JDBC driver JARs, and create JDBC connections.
---

Arch-GPT DB uses native Rust database drivers by default. JDBC is optional: install the JDBC plugin when you need to connect to a database that Arch-GPT DB does not support natively, or when you specifically need a vendor JDBC driver.

<Callout type="info">The JDBC plugin is not bundled with the main Arch-GPT DB app, and it does not include vendor JDBC drivers. Install the Arch-GPT DB JDBC plugin first, then import the database vendor's JDBC driver JAR.</Callout>

<Callout type="warn">A successful JDBC connection does not imply every capability of a native Arch-GPT DB profile. Object trees, source definitions, structure editing, import, transfer, and admin views depend on JDBC metadata, the detected effective database type, and the driver capability manifest. Unknown engines run with generic JDBC capabilities.</Callout>
...
Prefer built-in Arch-GPT DB database types first, such as MySQL, PostgreSQL, SQLite, Redis, MongoDB, DuckDB, ClickHouse, SQL Server, Oracle, DM, and GaussDB.

Use JDBC when:

- The target database is not built into Arch-GPT DB
...
| Item                   |         Required | Notes                                                                                  |
| ---------------------- | ---------------: | -------------------------------------------------------------------------------------- |
| Arch-GPT DB JDBC plugin        |              Yes | Installed from Arch-GPT DB settings; lets Arch-GPT DB call JDBC                                        |
...
If the command is missing, install Java first. Arch-GPT DB checks `JAVA_HOME` first, then common system locations and `java` from `PATH`.
...
<Tabs groupId="java-install" items={['macOS', 'Windows', 'Linux']}>
  ...
  <Tab value="Windows">
    Install Temurin, Oracle JDK, or Microsoft Build of OpenJDK. Restart Arch-GPT DB after installation and verify:
...
## Step 2: Install the Arch-GPT DB JDBC Plugin

In Arch-GPT DB Desktop:

1. Open **Settings**
2. Go to **JDBC Drivers**
3. Click **Install JDBC Plugin**

After installation, Arch-GPT DB shows a status like:

```text
Installed: v0.1.0
```

The Arch-GPT DB JDBC plugin has its own version. It does not have to match the Arch-GPT DB app version. Updating Arch-GPT DB does not automatically install the JDBC plugin, and if you uninstall it manually, Arch-GPT DB will not reinstall it behind your back.

## Step 3: Download the Vendor JDBC Driver

Arch-GPT DB does not download vendor JDBC drivers automatically. Download the `.jar` file from the database vendor website or a Maven repository.
...
You can also paste a local JAR path and import it. Arch-GPT DB copies imported JARs into the app data directory, and new JDBC connections can select them from the driver dropdown.
...
Arch-GPT DB already has a built-in DM ODBC connection type. If you prefer the official Dameng JDBC driver, use the JDBC plugin.

1. Install the Arch-GPT DB JDBC plugin
2. Install Java
...
Check whether your network can access Arch-GPT DB GitHub Releases. Arch-GPT DB tries proxy URLs and the original GitHub URL, but public proxies can still be unavailable. Try another network if GitHub is blocked.
...
If the terminal works but Arch-GPT DB does not, restart Arch-GPT DB. On macOS, if Java is only installed under a Homebrew path, configure `JAVA_HOME`.
...
Click **Uninstall** in **Settings → JDBC Drivers**. Arch-GPT DB removes the JDBC plugin itself but keeps your imported vendor driver JARs, so you do not have to import them again if you reinstall the plugin later.

## For Developers: Plugin Protocol

Arch-GPT DB starts the plugin executable and exchanges JSON Lines requests and responses over stdin/stdout. After a connection is established, Arch-GPT DB reuses the same plugin process for queries and metadata requests on that connection.
...
The current Arch-GPT DB app supports plugin protocol version `1`. Arch-GPT DB rejects a plugin whose manifest declares a different protocol version, and each plugin request has a 30 second timeout so a hung driver process does not block the app indefinitely.
