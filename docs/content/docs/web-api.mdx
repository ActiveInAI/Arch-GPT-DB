...
description: Internal HTTP APIs used by the PanDB Web and Docker UI, including authentication, queries, background jobs, uploads, and downloads.
---

<Callout type="warn">This API powers the PanDB Web UI and internal tooling such as the MCP Web backend. It is not a separate public integration contract. Endpoints and payloads may change between releases. For scripting and agent workflows, prefer [@dbx-app/cli](/en/docs/cli) or [@dbx-app/mcp-server](/en/docs/mcp) when possible.</Callout>

## Base URL

By default, PanDB Web listens on port `4224`:
...
| `DBX_PUBLIC_BASE_PATH` | Serve PanDB under a subpath such as `/dbx` |
...
Only the data-grid extractor currently exposes a tested, scoped OpenAPI document at `/api/query/data-grid-extractor-openapi.json`. It does not cover the complete PanDB Web API or version the other routes.
...
CLI supports the same `DBX_WEB_URL` and `DBX_WEB_PASSWORD` variables. Both integrations reuse server-side connections, drivers, read-only protection, production protection, and database privileges, making them easier to maintain across PanDB upgrades than copied internal route payloads.
...
- [examples/web-api/automation.sh](https://github.com/ActiveInAI/PanDB/tree/main/examples/web-api/automation.sh)
- [examples/docker/docker-compose.yml](https://github.com/ActiveInAI/PanDB/tree/main/examples/docker/docker-compose.yml)
- [examples/cli/basic-workflow.sh](https://github.com/ActiveInAI/PanDB/tree/main/examples/cli/basic-workflow.sh)
