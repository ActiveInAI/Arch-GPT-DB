---
title: Web API 参考
description: PanDB Web 与 Docker UI 使用的内部 HTTP API，包括认证、查询、后台任务、上传和下载边界。
---

<Callout type="warn">这套 API 主要服务于 PanDB Web 界面，以及 MCP Web 模式等内部工具。它不是单独对外承诺的集成契约，路由和字段可能随版本调整。做脚本或 Agent 集成时，优先使用 [@dbx-app/cli](/cn/docs/cli) 或 [@dbx-app/mcp-server](/cn/docs/mcp)。</Callout>

## 基础地址

PanDB Web 默认监听 `4224` 端口：
...
只有数据网格提取器当前提供受测试的局部 OpenAPI 文档：`/api/query/data-grid-extractor-openapi.json`。它不覆盖完整 PanDB Web API，也不代表其它路由已经版本化。
...
CLI 同样支持 `DBX_WEB_URL` 和 `DBX_WEB_PASSWORD`。两者都复用 Web 后端的连接、驱动、只读保护、生产保护和数据库权限，比复制内部路由字段更容易随 PanDB 升级。
