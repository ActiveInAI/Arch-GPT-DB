...
test("rollback dispatches each distribution workflow after validation", () => {
  ...
  assert.deepEqual(commands, [
    "workflow run publish-packages.yml --repo ActiveInAI/PanDB -f tag=v0.5.63 -f notify=false",
    "workflow run sync-cnb-release-assets.yml --repo ActiveInAI/PanDB -f tag=v0.5.63",
    "workflow run rollback-docker-latest.yml --repo ActiveInAI/PanDB -f tag=v0.5.63",
  ]);
});
