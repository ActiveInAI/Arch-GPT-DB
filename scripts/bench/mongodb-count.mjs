...
Options:
  --api-base=http://127.0.0.1:4224/api  PanDB Web API base URL
  ...
  --dbx-data-dir=/tmp/dbx-bench          Copy local Mongo agent jar into this PanDB data dir
...
Before running this benchmark, start PanDB Web with the same data dir, for example:
  DBX_DATA_DIR=/tmp/dbx-bench DBX_DISABLE_PASSWORD=1 cargo run -p dbx-web

The benchmark saves one temporary connection into that PanDB data dir, so use an
...
async function main() {
  ...
  if (!options.json) console.log("Connecting PanDB Web API...");
  ...
  const measurements = [
    ...
    await measureCase("PanDB find-documents total", options.iterations, options.warmups, () => measureDbxFindTotal(options, connectionId)),
    await measureCase("PanDB count-documents", options.iterations, options.warmups, () => measureDbxDedicatedCount(options, connectionId)),
  ];
