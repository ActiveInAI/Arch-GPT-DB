...
Options:
  ...
  --scan-count=10000                        PanDB SCAN COUNT value
  --max-iterations=15                       PanDB server-side SCAN iterations per API call
  --typed=false                             Skip PanDB includeTypes=true comparison
  --json                                    Print JSON only

Before running this benchmark, start PanDB Web separately, for example:
...
async function measureDbxScan(options, connectionId, includeTypes) {
  ...
  return {
    label: includeTypes ? "PanDB scan-keys-batch includeTypes=true" : "PanDB scan-keys-batch includeTypes=false",
    keys,
    calls,
    payloadBytes,
    elapsedMs: performance.now() - startedAt,
...
function printReport(result) {
  ...
  console.log(`- PanDB scan count: ${result.config.scanCount}`);
  console.log(`- PanDB max iterations: ${result.config.maxIterations}`);
...
async function main() {
  ...
  if (!options.json) {
    console.log("Connecting PanDB Web API...");
  }
