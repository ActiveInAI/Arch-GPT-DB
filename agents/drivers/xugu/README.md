...
Native XuguDB agent for PanDB using `gitee.com/XuguDB/go-xugu-driver`.
...
Build the binary, then copy it into PanDB's installed XuguDB driver directory:
...
PanDB prefers `agent` over `agent.jar`, so XuguDB connections will use this Go
