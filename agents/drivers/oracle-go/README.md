# PanDB Oracle Go Agent

Experimental native Oracle agent for PanDB using `github.com/sijms/go-ora/v2`.
...
Build the binary, then copy it into PanDB's installed Oracle driver directory:
...
PanDB prefers `agent` over `agent.jar`, so Oracle connections will use this Go
