...
func TestExecuteOracleSelectRetriesXMLTypeDecodeFailures(t *testing.T) {
	originalSQL := `SELECT * FROM (SELECT * FROM "PanDB"."TEST_LOBS") WHERE ROWNUM <= 100`
	rewrittenSQL := `SELECT * FROM (SELECT "ID", XMLSERIALIZE(CONTENT "XML_CONTENT" AS CLOB) AS "XML_CONTENT" FROM "PanDB"."TEST_LOBS") WHERE ROWNUM <= 100`
	calls := []string{}
...
func TestRewriteOracleXMLTypeNestedRownumQuery(t *testing.T) {
	sqlText, err := rewriteOracleXMLTypeSelectSQL(
		`SELECT * FROM (SELECT "ID", "XML_CONTENT" FROM "PanDB"."TEST_LOBS") WHERE ROWNUM <= 100`,
		fakeOracleColumnLoader([]oracleColumnMeta{
			{Name: "ID", DataType: "NUMBER"},
			{Name: "XML_CONTENT", DataType: "XMLTYPE"},
		}),
