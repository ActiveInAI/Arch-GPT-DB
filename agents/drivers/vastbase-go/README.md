...
This module implements the PanDB agent protocol for Vastbase with the pure-Go
...
Build the binary, then copy it into PanDB's installed Vastbase driver directory:
...
PanDB prefers `agent` over `agent.jar`. Remove the native binary to restore a
