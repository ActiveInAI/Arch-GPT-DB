...
Agent drivers for [PanDB](https://github.com/ActiveInAI/PanDB) — database support via JDBC and native database drivers.

Each agent runs as a standalone process and communicates with PanDB via stdin/stdout JSON-RPC 2.0.
...
Most Java agents target JRE 21. Native agents, such as `cassandra`, `duckdb`, `oracle`, `kingbase`, `tdengine`, `xugu`, and `rabbitmq`, do not require a JRE. PanDB downloads and manages the JRE 21 installation automatically for Java agents.
...
### Local PanDB Runtime Test

When changing a Java agent under `agents/drivers/<db_type>/` or shared Java agent protocol code, rebuild the target agent and replace the runtime JAR used by the local PanDB app:
...
Restart PanDB or disconnect and reconnect the database so the new agent process loads the replacement JAR.
...
[AGPL-3.0](https://github.com/ActiveInAI/PanDB/blob/main/LICENSE)
