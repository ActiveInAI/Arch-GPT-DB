...
PanDB 的 Agent 驱动 —— 通过 JDBC 和原生数据库驱动支持各种数据库。

每个 agent 作为独立进程运行，通过 stdin/stdout 与 PanDB 进行 JSON-RPC 2.0 通信。
...
多数 Java agent 以 JRE 21 为目标。原生 agent（如 `cassandra`、`duckdb`、`oracle`、`kingbase`、`tdengine`、`xugu` 和 `rabbitmq`）不需要 JRE。对 Java agent，PanDB 会自动下载并管理 JRE 21 安装。
...
HikariCP 会直接打进启用连接池的 Agent JAR。已经使用 PanDB 托管 JRE 21 的安装无需重新安装或替换 JRE。
...
修改 `agents/drivers/<db_type>/` 下的 Java agent 或共享 Java agent 协议代码后，需重新构建目标 agent 并替换本地 PanDB 应用使用的运行时 JAR：
...
重启 PanDB 或断开重连数据库，使新 agent 进程加载替换后的 JAR。
