...
export default {
  ...
  externalSqlFile: {
    modifiedTitle: "File changed outside Arch-GPT DB",
    ...
    deletedDescription: "{path} was deleted or moved outside Arch-GPT DB. The editor content is still available.",
  ...
  updates: {
    ...
    availableMessage: "Arch-GPT DB {latest} is available. You are using {current}.",
    upToDate: "Arch-GPT DB is up to date ({version}).",
    ...
    activeTasksBlockUpdate: "{count} task(s) are still running. Wait for them to finish before updating Arch-GPT DB.",
    portableAutomaticUpdate: "Arch-GPT DB will download the signed portable ZIP, replace only Arch-GPT DB.exe after exit, and restart automatically. portable.dbx and data will be kept.",
  ...
  sidebar: {
    ...
    importDbx: "Import Arch-GPT DB Config",
  ...
  sqlLibrary: {
    title: "SQL Library",
    storageHelp: "SQL Library content is stored in Arch-GPT DB app data, not as standalone .sql files on disk.\nTo use files in the file system, export the SQL Library.",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "Connected, but Arch-GPT DB could not remember the password: {message}",
    ...
    oracleTnsPathHint: "The Arch-GPT DB backend/Oracle agent reads this path. Desktop paths are local; Web and Docker paths are on the server.",
    ...
    kafkaKerberosKeytabPlaceholder: "Path on Arch-GPT DB Agent machine, e.g. /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "Optional path on Arch-GPT DB Agent machine, e.g. /etc/krb5.conf",
    kafkaKerberosPathHint: "The keytab and krb5.conf paths are read by Arch-GPT DB Agent and must exist on the machine running Arch-GPT DB Agent; files are not uploaded from this browser.",
    ...
    gaussdbConnectionModeNativeHint: "Uses Arch-GPT DB's built-in PostgreSQL-compatible protocol for GaussDB A/PG/Oracle compatibility modes.",
    ...
    agentJavaTooOld: "This driver requires Java 21. Use Arch-GPT DB managed JRE 21 or select a Java 21 executable in Driver Manager.",
    ...
    jdbcPluginHint: "Install the Arch-GPT DB JDBC plugin first. For drivers with external dependencies, install from Maven in Driver Manager or import the driver and all dependency JARs together.",
  ...
  editor: {
    ...
    unsavedChangesBatchCloseMultipleMessage: '{count} SQL tabs you are closing have unsaved changes. Arch-GPT DB is currently showing "{title}". Save before continuing?',
    unsavedChangesAppCloseMessage: '"{title}" has unsaved SQL changes that will be lost if you quit Arch-GPT DB. Save before quitting?',
    unsavedChangesAppCloseMultipleMessage: '{count} SQL tabs have unsaved changes. Arch-GPT DB is currently showing "{title}". Save before quitting?',
  ...
  tabs: {
    ...
    externalFileMissing: "Deleted or moved outside Arch-GPT DB",
  ...
  grid: {
    ...
    conditionalBulkEditConfirmMessage: "This executes immediately against every row matching the condition at execution time. Unloaded rows are included, and Arch-GPT DB cannot generate a reliable rollback for them.",
  ...
  configExport: {
    ...
    cryptoUnavailable: "Encrypted config import is unavailable in this browser context. Use HTTPS, localhost, or Arch-GPT DB Desktop.",
  ...
  ai: {
    ...
    closeWithRunsDescription: "Quitting Arch-GPT DB will interrupt {count} running or queued AI task(s). You can close this dialog and let them finish instead.",
    ...
    productionWriteBlocked: "AI cannot execute data or schema changes on a production database. Review this SQL and run it manually in Arch-GPT DB.",
    ...
    codexCliEnvReservedName: "{name} is managed by Arch-GPT DB for the MCP server and cannot be set here.",
    ...
    codexMcpRequiredTitle: "Arch-GPT DB MCP Server required",
    codexMcpRequiredDescription: "Codex CLI uses the Arch-GPT DB MCP Server to access database schema and query tools. Install it before using Codex in the AI assistant.",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "A Arch-GPT DB-managed MCP environment variable was overridden. Remove Arch-GPT DB_MCP_* variables from the provider configuration.",
      claudeCodeNotAuthenticated: "Claude Code CLI is not authenticated. Run `claude auth login` in a terminal and try again.",
      claudeCodeMcpConfigInvalid: "Claude Code could not read or parse the Arch-GPT DB MCP configuration. Check the Claude Code CLI version and executable path.",
      dbxMcpMissing: "A compatible Arch-GPT DB MCP Server installation was not found. Check Settings > MCP, then reinstall or update it.",
      claudeCodeMcpStartupFailed: "Claude Code loaded the MCP configuration but could not start Arch-GPT DB MCP Server. Check Settings > MCP and the diagnostics below.",
      claudeCodeCommandLineTooLong: "Windows rejected the Claude Code command because it was too long. Update Arch-GPT DB and retry using the generated MCP configuration file.",
      ...
      piAgentEnvReserved: "A Arch-GPT DB-managed Pi or MCP environment variable was overridden. Remove Arch-GPT DB_PI_* and Arch-GPT DB_MCP_* variables from the provider configuration.",
      ...
      piAgentMcpStartupFailed: "Pi could not start the scoped Arch-GPT DB MCP bridge. Check Settings > MCP and the diagnostics below.",
      ...
      openCodeEnvReserved: "An OpenCode or Arch-GPT DB-managed environment variable was overridden. Remove OPENCODE control variables and Arch-GPT DB_MCP_* variables from the provider configuration.",
      ...
      openCodeMcpStartupFailed: "OpenCode could not start the scoped Arch-GPT DB MCP server. Check Settings > MCP and the diagnostics below.",
      ...
      grokCliEnvReserved: "A Arch-GPT DB-managed MCP environment variable was overridden. Remove Arch-GPT DB_MCP_* variables from the provider configuration.",
      grokCliNotAuthenticated: "Grok CLI is not authenticated. Run `grok login` in a terminal and try again.",
      grokCliMcpStartupFailed: "Grok loaded the MCP configuration but could not start Arch-GPT DB MCP Server. Check Settings > MCP and the diagnostics below.",
      grokCliCommandLineTooLong: "Windows rejected the Grok CLI command because it was too long. Update Arch-GPT DB and retry using the generated prompt file.",
      ...
      codeBuddyEnvReserved: "A Arch-GPT DB-managed MCP environment variable was overridden. Remove Arch-GPT DB_MCP_* variables from the provider configuration.",
      ...
      codeBuddyMcpConfigInvalid: "CodeBuddy Code could not read the temporary Arch-GPT DB MCP configuration. Check the diagnostics below.",
      codeBuddyMcpStartupFailed: "CodeBuddy Code loaded the MCP configuration but could not start Arch-GPT DB MCP Server. Check Settings > MCP and the diagnostics below.",
      ...
      qoderEnvReserved: "A Arch-GPT DB-managed MCP environment variable was overridden. Remove Arch-GPT DB_MCP_* variables from the provider configuration.",
      ...
      qoderMcpConfigInvalid: "Qoder CLI could not read the temporary Arch-GPT DB MCP configuration. Check the Qoder CLI version and diagnostics below.",
      qoderMcpStartupFailed: "Qoder CLI loaded the MCP configuration but could not start Arch-GPT DB MCP Server. Check Settings > MCP and the diagnostics below.",
      ...
      cursorEnvReserved: "A Cursor or Arch-GPT DB-managed environment variable was overridden. Remove CURSOR_CONFIG_DIR, CURSOR_DATA_DIR, and Arch-GPT DB_MCP_* variables from the provider configuration.",
      cursorNotAuthenticated: "Cursor CLI is not authenticated. Run `agent login` in a terminal and try again.",
      cursorMcpStartupFailed: "Cursor could not start the scoped Arch-GPT DB MCP server. Check Settings > MCP and the diagnostics below.",
  ...
  contextMenu: {
    ...
    parameterDefaultHint: "When DEFAULT is checked, Arch-GPT DB omits that argument so the database uses the procedure default.",
  ...
  etcd: {
    ...
    access: {
      ...
      permissionUpdateHint: "etcd Auth has no atomic update. Saving revokes the old permission and then grants the new one. If granting fails, Arch-GPT DB attempts to restore the old permission.",
      saveChanges: "Save changes",
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "Cluster status remains available. Arch-GPT DB requests /metrics directly from the current etcd endpoint; verify that the resulting URL is reachable from the Arch-GPT DB application.",
  ...
  sqlParameters: {
    ...
    description: "Fill values for SQL template placeholders. Arch-GPT DB replaces them before executing the SQL.",
    name: "Parameter",
    syntax: "Syntax",
    type: "Type",
    value: "Value",
  ...
  settings: {
    ...
    uiFontAppDefault: "Arch-GPT DB Default",
    ...
    iconThemeDefaultDescription: "Use the color Arch-GPT DB app icon.",
    iconThemeBlack: "Black",
    iconThemeBlackDescription: "Use the black Arch-GPT DB logo in the app.",
    ...
    showTrayIconDescription: "When disabled, no icon is shown, but closing the window still hides Arch-GPT DB in the background as before.",
    quitOnClose: "Quit when closing window",
    quitOnCloseDescription: "When enabled, clicking the close button exits Arch-GPT DB completely instead of hiding it to the tray. You can change this later in Appearance settings.",
    ...
    closeActionQuit: "Quit Arch-GPT DB",
    closeActionMinimize: "Minimize to tray",
    updateNotificationsEnabled: "Enable update reminders",
    updateNotificationsEnabledDescription: "When disabled, Arch-GPT DB will not automatically check app or driver updates or show update badges. Manual checks are still available.",
    ...
    duckDbWorkerProcessIsolationDescription: "Runs DuckDB queries in a separate worker process. If a long query does not respond to cancellation, Arch-GPT DB can stop the worker process to release the database file lock.",
    duckDbWorkerMaxProcesses: "DuckDB max concurrent workers",
    duckDbWorkerMaxProcessesDescription: "Limits how many DuckDB worker processes Arch-GPT DB can run at the same time. Range: 1-16. Default: 4.",
    duckDbWorkerProcessIsolationRestartRequired: "Restart Arch-GPT DB for this change to take effect.",
    restartDbx: "Restart Arch-GPT DB",
    restartDbxFailed: "Failed to restart Arch-GPT DB: {error}",
    ...
    openTabsRestoreModeDescription: "Choose how Arch-GPT DB restores tabs that were open last time.",
    ...
    syncWebDavDescription: "Upload or restore a Arch-GPT DB snapshot, including saved SQL library entries, from a WebDAV-compatible storage service.",
    syncWebDavWebDescription: "In Web mode, the Arch-GPT DB server accesses WebDAV without browser CORS restrictions. Saved passwords remain in the current Arch-GPT DB instance.",
    ...
    syncRememberWebDavPasswordDescription: "The password is encrypted and stored in the current Arch-GPT DB instance only. It is not synced to WebDAV and does not replace the sync password.",
    syncRemotePath: "Remote snapshot path",
    syncRemotePathDescription: "Arch-GPT DB will create missing parent folders when uploading.",
    syncSecretNotice: "By default, Arch-GPT DB syncs connection details and settings only. Database passwords, SSH passwords, proxy passwords, HTTP tunnel tokens, connection strings, and AI API keys stay local.",
    syncSecrets: "Sync encrypted secrets",
    syncSecretsDescription: "When enabled, Arch-GPT DB encrypts database passwords, SSH passwords, proxy passwords, HTTP tunnel tokens, and AI API keys before uploading them to remote storage.",
    ...
    syncSecretsPassphraseDescription: "This password is only used to encrypt and restore sensitive data. It is encrypted in the current Arch-GPT DB instance; enter it again in other instances.",
    syncTest: "Test",
    syncUpload: "Upload",
    ...
    syncAutoUploadDescription: "Runs while Arch-GPT DB is open. Downloads still require manual confirmation to avoid replacing local saved SQL unexpectedly.",
    ...
    syncDownloadConfirm: "Download and apply the remote Arch-GPT DB snapshot? Local metadata and saved SQL will be replaced.",
    syncSnippetTitle: "GitHub / Gitee Snippet Sync",
    syncSnippetDescription: "Store the same Arch-GPT DB snapshot in a private code snippet. The first upload creates a snippet and saves its ID.",
    ...
    syncSnippetMigrateLegacyConfirm: "Arch-GPT DB will first create an encrypted replacement, then permanently delete legacy plaintext snippet {id}. This cannot be undone. Continue?",
    ...
    appCloseUnsavedTabsModeDescription: "Choose what happens when quitting Arch-GPT DB while SQL tabs have unsaved edits.",
    appCloseUnsavedTabsModePrompt: "Ask whether to save",
    appCloseUnsavedTabsModeKeepDrafts: "Quit and keep drafts",
    appCloseUnsavedTabsModeHint: 'Drafts are kept as unsaved tabs and restored the next time you start Arch-GPT DB. This requires "Restore tabs on launch" to not be set to "Do not keep tabs".',
    appCloseDraftPersistFailed: "Arch-GPT DB could not save the open tab drafts, so quitting was cancelled. Check disk space and file permissions, then try again. Details: {message}",
    ...
    sqlVariableSyntaxDescription: "Choose which variable and placeholder syntaxes Arch-GPT DB substitutes before running SQL, per database type. All are enabled by default.",
    ...
    jdbcPlugin: "Arch-GPT DB JDBC plugin",
    ...
    mcpTitle: "MCP Server",
    mcpDescription: "Check the Arch-GPT DB MCP Server installation and version used by Claude Code, Cursor, and other agents.",
    ...
    mcpUninstallConfirm: "Uninstall Arch-GPT DB MCP Server? Configured MCP clients will no longer be able to start it.",
    mcpUninstallFailed: "Uninstallation failed: {error}",
    mcpConfig: "MCP config",
    mcpConfigOptionsHint: "Arch-GPT DB centrally controls connection access and execution permissions for every MCP client. Generated configs contain only the runtime settings needed to start the MCP server and connect to this Arch-GPT DB instance, with no permission or connection-scope variables.",
    mcpScopeConnection: "Connections allowed for MCP",
    mcpScopeConnectionDescription: "Arch-GPT DB exposes only the selected connections to MCP. Changes apply to every MCP client.",
    ...
    mcpExecutionModeDescription: "Choose the permission level Arch-GPT DB enforces for every MCP client. Client-side read/write settings cannot change this level.",
    ...
    mcpExecutionModeReadOnlyDescription: "Allows only requests Arch-GPT DB classifies as reads; blocks recognized database writes and connection changes. Database-account permissions remain the final hard boundary.",
    ...
    mcpCapabilityDescription: "These are representative operations; requests Arch-GPT DB cannot classify reliably require full access.",
    ...
    mcpDetectionTiming: "Arch-GPT DB checks automatically when this page opens; use Check again to refresh.",
    mcpNpmBoundary: "Arch-GPT DB checks MCP status; installation, upgrades, and uninstallation run through the detected npm or pnpm runtime.",
    ...
    supportInfoAppVersion: "Arch-GPT DB Version",
  ...
  driverStore: {
    ...
    offlineExportDescription: "Select the Arch-GPT DB-managed Agent drivers to include. The package is compatible with the current platform only.",
    ...
    offlineExportNoDrivers: "No eligible Arch-GPT DB-managed Agent drivers are installed.",
    ...
    offlineExportSecurityHint: "Only Arch-GPT DB-managed artifacts and required managed JREs are included. Local paths, settings, and external drivers are excluded.",
    ...
    offlineExportUnavailable_unmanagedInstall: "This artifact is not tracked as a Arch-GPT DB-managed installation.",
    ...
    offlineExportUnavailable_missingManagedJre: "The required Arch-GPT DB-managed JRE is not installed.",
    offlineExportUnavailable_invalidManagedJre: "The required Arch-GPT DB-managed JRE is invalid or unsafe.",
    refresh: "Refresh",
    javaRuntime: "Java Runtime",
    javaRuntimeManaged: "Arch-GPT DB managed JRE",
    ...
    localInstallHint: "For air-gapped machines: on another computer with internet access, download the official dbx-jdbc-plugin-*.zip from the Arch-GPT DB GitHub Releases page, transfer it here, and select it — no network access is needed on this machine.",
  ...
  databaseBackup: {
    ...
    runtimeRequirement: "Scheduled backups run while Arch-GPT DB is open, including when it is minimized to the tray.",
  ...
  nacos: {
    ...
    protectedRole: "The built-in administrator role cannot be removed from Arch-GPT DB.",
    close: "Close",
    ...
    namespaceIdPlaceholder: "Leave empty for Arch-GPT DB to generate a UUID",
    namespaceName: "Namespace Name",
    ...
    nacosDetectAccessibleNamespacesHint: "Arch-GPT DB verifies configuration and service read access before offering namespaces for {name}.",
    ...
    nacosV3ConsoleManagedNamespacesHint: "This uses the Nacos 3 Console API (normally port 8080). Enter a namespace name or ID; Arch-GPT DB converts a name to its actual ID when it can read the directory.",
    ...
    persistentInstanceHint: "Arch-GPT DB does not register ephemeral instances because it cannot maintain their heartbeat.",
