...
export default {
  ...
  externalSqlFile: {
    modifiedTitle: "File changed outside PanDB",
    ...
    deletedDescription: "{path} was deleted or moved outside PanDB. The editor content is still available.",
  ...
  updates: {
    ...
    availableMessage: "PanDB {latest} is available. You are using {current}.",
    upToDate: "PanDB is up to date ({version}).",
    ...
    activeTasksBlockUpdate: "{count} task(s) are still running. Wait for them to finish before updating PanDB.",
    portableAutomaticUpdate: "PanDB will download the signed portable ZIP, replace only PanDB.exe after exit, and restart automatically. portable.pandb and data will be kept.",
  ...
  connection: {
    ...
    oracleTnsPathHint: "The PanDB backend/Oracle agent reads this path. Desktop paths are local; Web and Docker paths are on the server.",
    ...
    kafkaKerberosKeytabPlaceholder: "Path on PanDB Agent machine, e.g. /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "Optional path on PanDB Agent machine, e.g. /etc/krb5.conf",
    kafkaKerberosPathHint: "The keytab and krb5.conf paths are read by PanDB Agent and must exist on the machine running PanDB Agent; files are not uploaded from this browser.",
    ...
    gaussdbConnectionModeNativeHint: "Uses PanDB's built-in PostgreSQL-compatible protocol for GaussDB A/PG/Oracle compatibility modes.",
    ...
    agentJavaTooOld: "This driver requires Java 21. Use PanDB managed JRE 21 or select a Java 21 executable in Driver Manager.",
    ...
    jdbcPluginHint: "Install the PanDB JDBC plugin first. For drivers with external dependencies, install from Maven in Driver Manager or import the driver and all dependency JARs together.",
  ...
  editor: {
    ...
    unsavedChangesBatchCloseMultipleMessage: '{count} SQL tabs you are closing have unsaved changes. PanDB is currently showing "{title}". Save before continuing?',
    unsavedChangesAppCloseMessage: '"{title}" has unsaved SQL changes that will be lost if you quit PanDB. Save before quitting?',
    unsavedChangesAppCloseMultipleMessage: '{count} SQL tabs have unsaved changes. PanDB is currently showing "{title}". Save before quitting?',
  ...
  tabs: {
    ...
    externalFileMissing: "Deleted or moved outside PanDB",
  ...
  configExport: {
    ...
    cryptoUnavailable: "Encrypted config import is unavailable in this browser context. Use HTTPS, localhost, or PanDB Desktop.",
  ...
  ai: {
    ...
    codexCliEnvReservedName: "{name} is managed by PanDB for the MCP server and cannot be set here.",
    ...
    codexMcpRequiredTitle: "PanDB MCP Server required",
    codexMcpRequiredDescription: "Codex CLI uses the PanDB MCP Server to access database schema and query tools. Install it before using Codex in the AI assistant.",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "A PanDB-managed MCP environment variable was overridden. Remove DBX_MCP_* variables from the provider configuration.",
      claudeCodeNotAuthenticated: "Claude Code CLI is not authenticated. Run `claude auth login` in a terminal and try again.",
      claudeCodeMcpConfigInvalid: "Claude Code could not read or parse the PanDB MCP configuration. Check the Claude Code CLI version and executable path.",
      dbxMcpMissing: "A compatible PanDB MCP Server installation was not found. Check Settings > MCP, then reinstall or update it.",
      claudeCodeMcpStartupFailed: "Claude Code loaded the MCP configuration but could not start PanDB MCP Server. Check Settings > MCP and the diagnostics below.",
      claudeCodeCommandLineTooLong: "Windows rejected the Claude Code command because it was too long. Update PanDB and retry using the generated MCP configuration file.",
      ...
      piAgentEnvReserved: "A PanDB-managed Pi or MCP environment variable was overridden. Remove DBX_PI_* and DBX_MCP_* variables from the provider configuration.",
      ...
      piAgentMcpStartupFailed: "Pi could not start the scoped PanDB MCP bridge. Check Settings > MCP and the diagnostics below.",
  ...
  contextMenu: {
    ...
    parameterDefaultHint: "When DEFAULT is checked, PanDB omits that argument so the database uses the procedure default.",
  ...
  etcd: {
    ...
    access: {
      ...
      permissionUpdateHint: "etcd Auth has no atomic update. Saving revokes the old permission and then grants the new one. If granting fails, PanDB attempts to restore the old permission.",
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "Cluster status remains available. PanDB requests /metrics directly from the current etcd endpoint; verify that the resulting URL is reachable from the PanDB application.",
  ...
  sqlParameters: {
    ...
    description: "Fill values for SQL template placeholders. PanDB replaces them before executing the SQL.",
    name: "Parameter",
    syntax: "Syntax",
    type: "Type",
    value: "Value",
  ...
  settings: {
    ...
    uiFontAppDefault: "PanDB Default",
    ...
    iconThemeDefaultDescription: "Use the color PanDB app icon.",
    iconThemeBlack: "Black",
    iconThemeBlackDescription: "Use the black PanDB logo in the app.",
    ...
    showTrayIconDescription: "When disabled, no icon is shown, but closing the window still hides PanDB in the background as before.",
    quitOnClose: "Quit when closing window",
    quitOnCloseDescription: "When enabled, clicking the close button exits PanDB completely instead of hiding it to the tray. You can change this later in Appearance settings.",
    ...
    closeActionQuit: "Quit PanDB",
    closeActionMinimize: "Minimize to tray",
    updateNotificationsEnabled: "Enable update reminders",
    updateNotificationsEnabledDescription: "When disabled, PanDB will not automatically check app or driver updates or show update badges. Manual checks are still available.",
    ...
    duckDbWorkerProcessIsolationDescription: "Runs DuckDB queries in a separate worker process. If a long query does not respond to cancellation, PanDB can stop the worker process to release the database file lock.",
    duckDbWorkerMaxProcesses: "DuckDB max concurrent workers",
    duckDbWorkerMaxProcessesDescription: "Limits how many DuckDB worker processes PanDB can run at the same time. Range: 1-16. Default: 4.",
    duckDbWorkerProcessIsolationRestartRequired: "Restart PanDB for this change to take effect.",
    restartDbx: "Restart PanDB",
    restartDbxFailed: "Failed to restart PanDB: {error}",
    ...
    openTabsRestoreModeDescription: "Choose how PanDB restores tabs that were open last time.",
    ...
    syncWebDavDescription: "Upload or restore a PanDB snapshot, including saved SQL library entries, from a WebDAV-compatible storage service.",
    ...
    syncRemotePathDescription: "PanDB will create missing parent folders when uploading.",
    syncSecretNotice: "By default, PanDB syncs connection details and settings only. Database passwords, SSH passwords, proxy passwords, HTTP tunnel tokens, connection strings, and AI API keys stay local.",
    syncSecrets: "Sync encrypted secrets",
    syncSecretsDescription: "When enabled, PanDB encrypts database passwords, SSH passwords, proxy passwords, HTTP tunnel tokens, and AI API keys before uploading them to remote storage.",
    ...
    syncSecretsPassphraseDescription: "This password is only used to encrypt and restore sensitive data. PanDB does not save it; enter it again on each device.",
    syncTest: "Test",
    syncUpload: "Upload",
    ...
    syncAutoUploadDescription: "Runs while PanDB is open. Downloads still require manual confirmation to avoid replacing local saved SQL unexpectedly.",
    ...
    syncDownloadConfirm: "Download and apply the remote PanDB snapshot? Local metadata and saved SQL will be replaced.",
    syncSnippetTitle: "GitHub / Gitee Snippet Sync",
    syncSnippetDescription: "Store the same PanDB snapshot in a private code snippet. The first upload creates a snippet and saves its ID.",
    ...
    syncSnippetMigrateLegacyConfirm: "PanDB will first create an encrypted replacement, then permanently delete legacy plaintext snippet {id}. This cannot be undone. Continue?",
    ...
    sqlVariableSyntaxDescription: "Choose which variable and placeholder syntaxes PanDB substitutes before running SQL, per database type. All are enabled by default.",
    ...
    mcpTitle: "MCP Server",
    mcpDescription: "Check the PanDB MCP Server installation and version used by Claude Code, Cursor, and other agents.",
    ...
    mcpConfigOptionsHint: "PanDB centrally controls connection access and execution permissions for every MCP client. Generated configs contain only the runtime settings needed to start the MCP server and connect to this PanDB instance, with no permission or connection-scope variables.",
    mcpScopeConnection: "Connections allowed for MCP",
    mcpScopeConnectionDescription: "PanDB exposes only the selected connections to MCP. Changes apply to every MCP client.",
    ...
    mcpExecutionModeDescription: "Choose the permission level PanDB enforces for every MCP client. Client-side read/write settings cannot change this level.",
    ...
    mcpExecutionModeReadOnlyDescription: "Allows only requests PanDB classifies as reads; blocks recognized database writes and connection changes. Database-account permissions remain the final hard boundary.",
    ...
    mcpCapabilityDescription: "These are representative operations; requests PanDB cannot classify reliably require full access.",
    ...
    mcpDetectionTiming: "PanDB checks automatically when this page opens; use Check again to refresh.",
    mcpNpmBoundary: "PanDB only checks and explains MCP status; installation and upgrades still run through npm.",
    ...
    supportInfoAppVersion: "PanDB Version",
  ...
  driverStore: {
    ...
    javaRuntimeManaged: "PanDB managed JRE",
  ...
  databaseBackup: {
    ...
    runtimeRequirement: "Scheduled backups run while PanDB is open, including when it is minimized to the tray.",
  ...
  nacos: {
    ...
    persistentInstanceHint: "PanDB does not register ephemeral instances because it cannot maintain their heartbeat.",
