...
export default withEnglishFallback({
  ...
  updates: {
    ...
    portableAutomaticUpdate: "Arch-GPT DB は署名済みのポータブル ZIP をダウンロードし、終了後に Arch-GPT DB.exe のみを置き換えて自動的に再起動します。portable.pandb とデータは保持されます。",
  ...
  ai: {
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "Arch-GPT DBが管理するMCP環境変数が上書きされています。プロバイダー設定からDBX_MCP_*変数を削除してください。",
      ...
      piAgentEnvReserved: "Arch-GPT DBが管理するPiまたはMCP環境変数が上書きされています。DBX_PI_*とDBX_MCP_*を削除してください。",
  ...
  settings: {
    ...
    syncSecretsPassphraseDescription: "このパスワードは機密データの暗号化と復元にのみ使用されます。Arch-GPT DBは保存せず、各デバイスで再度入力する必要があります。",
    syncTest: "テスト",
    syncUpload: "アップロード",
    syncDownload: "ダウンロード",
