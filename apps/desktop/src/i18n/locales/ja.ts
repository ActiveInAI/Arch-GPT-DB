...
export default withEnglishFallback({
  ...
  externalSqlFile: {
    modifiedTitle: "ファイルがArch-GPT DBの外部で変更されました",
    modifiedDescription: "{path} は別のプログラムによって変更されました。保持するバージョンを選択してください。",
    deletedTitle: "ファイルが存在しません",
    deletedDescription: "{path} はArch-GPT DBの外部で削除または移動されました。エディターの内容は保持されています。",
  ...
  updates: {
    ...
    availableMessage: "Arch-GPT DB {latest} が利用可能です。現在のバージョンは {current} です。",
    upToDate: "Arch-GPT DB は最新です ({version})。",
    ...
    activeTasksBlockUpdate: "{count} 件のタスクが実行中です。完了してから Arch-GPT DB を更新してください。",
    portableAutomaticUpdate: "Arch-GPT DB は署名済みのポータブル ZIP をダウンロードし、終了後に Arch-GPT DB.exe のみを置き換えて自動的に再起動します。portable.dbx とデータは保持されます。",
  ...
  sqlLibrary: {
    title: "SQLライブラリ",
    storageHelp: "SQLライブラリの内容はArch-GPT DBのアプリデータに保存され、ディスク上の独立した.sqlファイルではありません。\nファイルシステムで使用するには、SQLライブラリをエクスポートしてください。",
    unassociated: "未関連付け",
  ...
  connection: {
    ...
    rememberPasswordSaveFailed: "接続しましたが、Arch-GPT DB はパスワードを保存できませんでした: {message}",
    ...
    oracleTnsPathHint: "このパスは Arch-GPT DB バックエンド/Oracle Agent が読み取ります。デスクトップではローカルパス、Web/Docker ではサーバーパスを指定してください。",
    ...
    gaussdbConnectionModeNativeHint: "GaussDB の A/PG/Oracle 互換モードでは、Arch-GPT DB 内蔵の PostgreSQL 互換プロトコルを使用します。",
    ...
    agentJavaTooOld: "このドライバーにはJava 21が必要です。ドライバーマネージャーでArch-GPT DB管理JRE 21を使用するか、Java 21実行ファイルを選択してください。",
    ...
    jdbcPluginHint: "最初にArch-GPT DB JDBCプラグインをインストールし、次にデータベースベンダーのJDBCドライバーJARをインポートしてください。",
    ...
    kafkaKerberosKeytabPlaceholder: "Arch-GPT DB Agent が動作するマシンのパス。例: /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "オプション。Arch-GPT DB Agent が動作するマシンのパス。例: /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab と krb5.conf のパスは Arch-GPT DB Agent が読み取るため、Arch-GPT DB Agent が動作するマシン上に存在する必要があります。ブラウザからファイルをアップロードすることはありません。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」に未保存の SQL 変更があり、Arch-GPT DB の終了後に失われます。保存してから終了しますか？",
    unsavedChangesAppCloseMultipleMessage: "{count} 個の SQL タブが未保存です。現在「{title}」を表示中です。Arch-GPT DB の終了後に失われます。保存してから終了しますか？",
  ...
  tabs: {
    ...
    externalFileMissing: "Arch-GPT DBの外部で削除または移動されました",
  ...
  grid: {
    ...
    conditionalBulkEditConfirmMessage: "実行時に条件へ一致するすべての行（未読込の行を含む）を直ちに更新します。Arch-GPT DB は信頼できるロールバック SQL を生成できません。",
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "Arch-GPT DB はこの結果に対して安全に書き戻せる対象テーブルを 1 つに特定できませんでした。",
      ...
      "metadata-unavailable": "Arch-GPT DBがテーブルメタデータを読み込めなかったため、結果の編集は無効です。",
    },
  ...
  configExport: {
    ...
    cryptoUnavailable: "暗号化設定のインポートはこのブラウザ環境では利用できません。HTTPS、localhost、またはArch-GPT DB Desktopを使用してください。",
  ...
  ai: {
    ...
    codexMcpRequiredTitle: "Arch-GPT DB MCP Serverが必要です",
    codexMcpRequiredDescription: "Codex CLIはArch-GPT DB MCP Server経由でデータベーススキーマとクエリツールにアクセスします。AIアシスタントでCodexを使う前にインストールしてください。",
    ...
    codexCliEnvReservedName: "{name}はMCP Server用にArch-GPT DBが管理しているため、ここでは設定できません。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "Arch-GPT DBが管理するMCP環境変数が上書きされています。プロバイダー設定からArch-GPT DB_MCP_*変数を削除してください。",
      claudeCodeNotAuthenticated: "Claude Code CLIにログインしていません。ターミナルで`claude auth login`を実行してから再試行してください。",
      claudeCodeMcpConfigInvalid: "Claude CodeがArch-GPT DBのMCP設定を読み取りまたは解析できません。Claude Code CLIのバージョンと実行ファイルのパスを確認してください。",
      dbxMcpMissing: "互換性のあるArch-GPT DB MCP Serverが見つかりません。設定 > MCPで確認し、再インストールまたは更新してください。",
      claudeCodeMcpStartupFailed: "Claude CodeはMCP設定を読み込みましたが、Arch-GPT DB MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      claudeCodeCommandLineTooLong: "Windowsが長すぎるClaude Codeコマンドを拒否しました。Arch-GPT DBを更新し、生成されたMCP設定ファイルで再試行してください。",
      ...
      piAgentEnvReserved: "Arch-GPT DBが管理するPiまたはMCP環境変数が上書きされています。Arch-GPT DB_PI_*とArch-GPT DB_MCP_*を削除してください。",
      piAgentNotAuthenticated: "Pi Coding Agentに認証済みのモデルプロバイダーがありません。Piでプロバイダーを設定またはログインしてください。",
      piAgentMcpStartupFailed: "PiはArch-GPT DB MCPブリッジを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      openCodeEnvReserved: "OpenCodeまたはArch-GPT DBが管理する環境変数が上書きされています。設定からOPENCODE制御変数とArch-GPT DB_MCP_*を削除してください。",
      openCodeNotAuthenticated: "OpenCodeに認証済みのモデルプロバイダーがありません。`opencode auth login`を実行するかOpenCodeでプロバイダーを設定して、再試行してください。",
      openCodeMcpStartupFailed: "OpenCodeはスコープ付きArch-GPT DB MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      cursorEnvReserved: "CursorまたはArch-GPT DBが管理する環境変数が上書きされています。CURSOR_CONFIG_DIR、CURSOR_DATA_DIR、Arch-GPT DB_MCP_*を設定から削除してください。",
      cursorNotAuthenticated: "Cursor CLIにログインしていません。ターミナルで`agent login`を実行してから再試行してください。",
      cursorMcpStartupFailed: "Cursorはスコープ付きArch-GPT DB MCPサーバーを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
      ...
      grokCliEnvReserved: "Arch-GPT DBが管理するMCP環境変数が上書きされています。プロバイダー設定からArch-GPT DB_MCP_*変数を削除してください。",
      grokCliNotAuthenticated: "Grok CLIにログインしていません。ターミナルで`grok login`を実行してから再試行してください。",
      grokCliMcpStartupFailed: "GrokはMCP設定を読み込みましたが、Arch-GPT DB MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      grokCliCommandLineTooLong: "Windowsが長すぎるGrok CLIコマンドを拒否しました。Arch-GPT DBを更新し、生成されたプロンプトファイルで再試行してください。",
      ...
      codeBuddyEnvReserved: "Arch-GPT DBが管理するMCP環境変数が上書きされています。プロバイダー設定からArch-GPT DB_MCP_*変数を削除してください。",
      ...
      codeBuddyMcpConfigInvalid: "CodeBuddy Codeは一時的なArch-GPT DB MCP設定を読み込めませんでした。以下の診断情報を確認してください。",
      codeBuddyMcpStartupFailed: "CodeBuddy CodeはMCP設定を読み込みましたが、Arch-GPT DB MCP Serverを起動できません。設定 > MCPと以下の診断情報を確認してください。",
      ...
      qoderEnvReserved: "Arch-GPT DBが管理するMCP環境変数が上書きされました。プロバイダー設定からArch-GPT DB_MCP_*変数を削除してください。",
      ...
      qoderMcpConfigInvalid: "Qoder CLIが一時的なArch-GPT DB MCP設定を読み取れませんでした。Qoder CLIのバージョンと以下の診断詳細を確認してください。",
      qoderMcpStartupFailed: "Qoder CLIはMCP設定を読み込みましたが、Arch-GPT DB MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
    ...
    closeWithRunsDescription: "Arch-GPT DBを終了すると、実行中またはキューに入っている{count}件のAIタスクが中断されます。このダイアログを閉じて完了を待つこともできます。",
    ...
    productionWriteBlocked: "AI は本番データベースでデータまたはスキーマの変更を実行できません。この SQL を確認し、Arch-GPT DB で手動実行してください。",
  ...
  contextMenu: {
    ...
    parameterDefaultHint: "DEFAULTがチェックされている場合、Arch-GPT DBはその引数を省略し、データベースがプロシージャのデフォルト値を使用します。",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "クラスタ状態は引き続き利用できます。Arch-GPT DB は現在の etcd エンドポイントへ直接 /metrics を要求するため、生成された URL に Arch-GPT DB アプリケーションから到達できることを確認してください。",
  ...
  sqlParameters: {
    title: "SQLパラメーター",
    description: "SQLテンプレートのプレースホルダーに値を入力します。Arch-GPT DBは実行前にSQLリテラルへ置換します。",
    name: "パラメーター",
    syntax: "Syntax",
    type: "型",
    value: "値",
  ...
  settings: {
    ...
    uiFontAppDefault: "Arch-GPT DB デフォルト",
    ...
    iconThemeDefaultDescription: "カラーArch-GPT DBアプリアイコンを使用。",
    iconThemeBlack: "黒",
    iconThemeBlackDescription: "アプリ内で黒いArch-GPT DBロゴを使用。",
    iconThemeBlackDescriptionMac: "アプリ内は黒いロゴ、Dockは黒い背景、メニューバーはシステムに従います。",
    showTrayIcon: "トレイ/メニューバーアイコンを表示",
    showTrayIconDescription: "無効時はアイコンが表示されませんが、ウィンドウを閉じると従来通りArch-GPT DBはバックグラウンドに隠れます。",
    quitOnClose: "ウィンドウを閉じるときに終了",
    quitOnCloseDescription: "有効にすると、閉じるボタンでトレイに隠すのではなく、Arch-GPT DBを完全に終了します。",
    closeActionPromptTitle: "ウィンドウを閉じる",
    closeActionPromptDescription: "閉じるボタンの動作を選択してください。後から「設定 → 外観」で変更できます。",
    closeActionQuit: "Arch-GPT DBを終了",
    closeActionMinimize: "トレイに最小化",
    updateNotificationsEnabled: "アップデート通知を有効にする",
    updateNotificationsEnabledDescription: "無効時、Arch-GPT DBはアプリやドライバーのアップデートを自動確認せず、アップデートバッジも表示しません。手動確認は引き続き利用可能です。",
    ...
    duckDbWorkerProcessIsolationDescription: "DuckDB クエリを独立した worker プロセスで実行します。長時間クエリがキャンセルに応答しない場合、Arch-GPT DB は worker プロセスを停止してデータベースファイルのロックを解放できます。",
    duckDbWorkerMaxProcesses: "DuckDB の最大同時 worker 数",
    duckDbWorkerMaxProcessesDescription: "Arch-GPT DB が同時に実行できる DuckDB worker プロセス数を制限します。範囲: 1-16。既定値: 4。",
    duckDbWorkerProcessIsolationRestartRequired: "この変更を有効にするには Arch-GPT DB を再起動してください。",
    restartDbx: "Arch-GPT DB を再起動",
    restartDbxFailed: "Arch-GPT DB の再起動に失敗しました: {error}",
    ...
    openTabsRestoreModeDescription: "Arch-GPT DB 起動時に前回開いていたタブをどのように復元するかを選択します。",
    ...
    syncWebDavDescription: "保存済みSQLライブラリを含むArch-GPT DBスナップショットをWebDAV互換ストレージへアップロードまたは復元します。",
    syncWebDavWebDescription: "WebモードではArch-GPT DBサーバーがWebDAVへ接続するため、ブラウザーのCORS制限を受けません。保存したパスワードは現在のArch-GPT DBインスタンス内に保持されます。",
    ...
    syncRememberWebDavPasswordDescription: "パスワードは暗号化され、現在のArch-GPT DBインスタンスにのみ保存されます。WebDAVには同期されず、同期パスワードを置き換えるものでもありません。",
    syncRemotePath: "リモートスナップショットパス",
    syncRemotePathDescription: "アップロード時に不足している親フォルダをArch-GPT DBが作成します。",
    syncSecretNotice: "デフォルトでは、Arch-GPT DBは接続詳細と設定のみを同期します。データベースパスワード、SSHパスワード、プロキシパスワード、接続文字列、AI APIキーはローカルに保持されます。",
    syncSecrets: "暗号化されたシークレットを同期",
    syncSecretsDescription: "有効時、Arch-GPT DBはデータベースパスワード、SSHパスワード、AI APIキーを暗号化してからWebDAVにアップロードします。",
    syncSecretsPassphrase: "同期パスワード",
    syncSecretsPassphraseDescription: "このパスワードは機密データの暗号化と復元にのみ使用され、現在のArch-GPT DBインスタンスに暗号化して保存されます。他のインスタンスでは再入力してください。",
    syncTest: "テスト",
    syncUpload: "アップロード",
    syncDownload: "ダウンロード",
    ...
    syncAutoUploadDescription: "Arch-GPT DBを開いている間だけ実行されます。ローカルの保存済みSQLを誤って置き換えないよう、ダウンロードは手動確認が必要です。",
    ...
    syncDownloadConfirm: "リモートArch-GPT DBスナップショットをダウンロードして適用しますか？ローカルのメタデータと保存済みSQLが置き換えられます。",
    apply: "適用",
    ...
    appCloseUnsavedTabsModeDescription: "未保存の編集があるSQLタブが開いたままArch-GPT DBを終了するときの動作を選択します。",
    appCloseUnsavedTabsModePrompt: "保存するかどうか確認する",
    appCloseUnsavedTabsModeKeepDrafts: "確認せず終了して下書きを保持",
    appCloseUnsavedTabsModeHint: "下書きは未保存のタブとして保持され、次回Arch-GPT DBを起動したときに復元されます。「起動時にタブを復元」が「タブを保持しない」になっていない必要があります。",
    appCloseDraftPersistFailed: "開いているタブの下書きを保存できなかったため、Arch-GPT DB の終了を中止しました。ディスク容量とファイル権限を確認してから、もう一度終了してください。詳細: {message}",
    ...
    jdbcPlugin: "Arch-GPT DB JDBCプラグイン",
    ...
    mcpTitle: "MCPサーバー",
    mcpDescription: "Claude Code、Cursor、その他のエージェントが使用するArch-GPT DB MCPサーバーのインストールとバージョンを確認します。",
    mcpChecking: "確認中",
    ...
    mcpConfig: "MCP設定",
    mcpConfigOptionsHint: "接続範囲と実行権限はArch-GPT DBがすべてのMCPクライアントに一元適用します。生成される設定にはMCP Serverの起動と現在のArch-GPT DBへの接続に必要な実行パラメーターのみが含まれ、権限や接続範囲の変数は含まれません。",
    mcpScopeConnection: "MCPに許可する接続",
    mcpScopeConnectionDescription: "Arch-GPT DBは選択した接続だけをMCPに公開します。変更はすべてのMCPクライアントに適用されます。",
    ...
    mcpExecutionModeDescription: "Arch-GPT DBがすべてのMCPクライアントに適用する権限レベルを選択します。クライアント側の読み書き権限設定では、このレベルを変更できません。",
    ...
    mcpExecutionModeReadOnlyDescription: "Arch-GPT DBが読み取りと判定したリクエストのみを許可し、識別できるデータベース書き込みと接続変更を拒否します。データベースアカウントの権限が最終的な安全境界です。",
    ...
    mcpCapabilityDescription: "代表的な操作を示しています。Arch-GPT DBが確実に分類できないリクエストにはフルアクセスが必要です。",
    ...
    mcpDetectionTiming: "このページを開くとArch-GPT DBが自動的に確認します。更新するには再確認を使用してください。",
    mcpNpmBoundary: "Arch-GPT DBはMCPステータスの確認と説明のみを行います。インストールとアップグレードはnpmを通じて実行されます。",
    mcpRefresh: "再確認",
    mcpGuide: "MCPガイド",
    ...
    supportInfoAppVersion: "Arch-GPT DBバージョン",
    ...
    syncSnippetDescription: "同一の Arch-GPT DB スナップショットを非公開スニペットに保存します。初回アップロード時にスニペットが自動作成され、ID が保存されます。",
    ...
    syncSnippetMigrateLegacyConfirm: "Arch-GPT DB はまず暗号化された置換スニペットを作成し、その後、旧式の平文スニペット {id} を完全に削除します。この操作は取り消せません。続行しますか？",
  ...
  driverStore: {
    ...
    offlineExportDescription: "含めるArch-GPT DB管理のAgentドライバーを選択します。パッケージは現在のプラットフォームでのみ使用できます。",
    ...
    offlineExportNoDrivers: "エクスポート可能なArch-GPT DB管理のAgentドライバーがインストールされていません。",
    ...
    offlineExportSecurityHint: "Arch-GPT DB管理の成果物と必要なJREのみを含めます。ローカルパス、設定、外部ドライバーは除外されます。",
    ...
    offlineExportUnavailable_unmanagedInstall: "この成果物はArch-GPT DB管理のインストールとして記録されていません。",
    ...
    offlineExportUnavailable_missingManagedJre: "必要なArch-GPT DB管理JREがインストールされていません。",
    offlineExportUnavailable_invalidManagedJre: "必要なArch-GPT DB管理JREが無効または安全ではありません。",
    refresh: "更新",
    javaRuntime: "Javaランタイム",
    javaRuntimeManaged: "Arch-GPT DB管理JRE",
    ...
    localInstallHint: "完全に隔離されたネットワーク環境の場合：インターネットに接続できる別のコンピューターで Arch-GPT DB の GitHub Releases ページから公式の dbx-jdbc-plugin-*.zip をダウンロードし、このマシンに転送して選択してください。このマシンはネットワークに接続する必要はありません。",
  ...
  nacos: {
    ...
    namespaceId: "名前空間ID",
    namespaceIdPlaceholder: "空欄の場合はArch-GPT DBがUUIDを生成",
    namespaceName: "名前空間名",
    ...
    nacosDetectAccessibleNamespacesHint: "Arch-GPT DB は {name} の名前空間を提示する前に、設定とサービスの読み取りアクセスを検証します。",
    ...
    nacosV3ConsoleManagedNamespacesHint: "この接続は Nacos 3 Console API（通常はポート 8080）を使用します。名前空間名または ID を入力できます。ディレクトリを読み取れる場合、Arch-GPT DB は名前を実際の ID に変換します。",
    ...
    persistentInstanceHint: "Arch-GPT DB はハートビートを維持できないため、一時インスタンスを登録しません。",
    ipAddress: "IP アドレス",
    port: "ポート",
  ...
  databaseBackup: {
    ...
    runtimeRequirement: "Arch-GPT DBが開いている状態（トレイに最小化したときも含む）で、定期バックアップが実行されます。",
