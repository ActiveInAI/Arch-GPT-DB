...
export default withEnglishFallback({
  ...
  externalSqlFile: {
    modifiedTitle: "ファイルがPanDBの外部で変更されました",
    modifiedDescription: "{path} は別のプログラムによって変更されました。保持するバージョンを選択してください。",
    deletedTitle: "ファイルが存在しません",
    deletedDescription: "{path} はPanDBの外部で削除または移動されました。エディターの内容は保持されています。",
  ...
  updates: {
    ...
    availableMessage: "PanDB {latest} が利用可能です。現在のバージョンは {current} です。",
    upToDate: "PanDB は最新です ({version})。",
    ...
    activeTasksBlockUpdate: "{count} 件のタスクが実行中です。完了してから PanDB を更新してください。",
    portableAutomaticUpdate: "PanDB は署名済みのポータブル ZIP をダウンロードし、終了後に PanDB.exe のみを置き換えて自動的に再起動します。portable.pandb とデータは保持されます。",
  ...
  connection: {
    ...
    oracleTnsPathHint: "このパスは PanDB バックエンド/Oracle Agent が読み取ります。デスクトップではローカルパス、Web/Docker ではサーバーパスを指定してください。",
    ...
    gaussdbConnectionModeNativeHint: "GaussDB の A/PG/Oracle 互換モードでは、PanDB 内蔵の PostgreSQL 互換プロトコルを使用します。",
    ...
    agentJavaTooOld: "このドライバーにはJava 21が必要です。ドライバーマネージャーでPanDB管理JRE 21を使用するか、Java 21実行ファイルを選択してください。",
    ...
    jdbcPluginHint: "最初にPanDB JDBCプラグインをインストールし、次にデータベースベンダーのJDBCドライバーJARをインポートしてください。",
    ...
    kafkaKerberosKeytabPlaceholder: "PanDB Agent が動作するマシンのパス。例: /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "オプション。PanDB Agent が動作するマシンのパス。例: /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab と krb5.conf のパスは PanDB Agent が読み取るため、PanDB Agent が動作するマシン上に存在する必要があります。ブラウザからファイルをアップロードすることはありません。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」に未保存の SQL 変更があり、PanDB の終了後に失われます。保存してから終了しますか？",
    unsavedChangesAppCloseMultipleMessage: "{count} 個の SQL タブが未保存です。現在「{title}」を表示中です。PanDB の終了後に失われます。保存してから終了しますか？",
  ...
  tabs: {
    ...
    externalFileMissing: "PanDBの外部で削除または移動されました",
  ...
  grid: {
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "PanDB はこの結果に対して安全に書き戻せる対象テーブルを 1 つに特定できませんでした。",
      ...
      "metadata-unavailable": "PanDBがテーブルメタデータを読み込めなかったため、結果の編集は無効です。",
    },
  ...
  configExport: {
    ...
    cryptoUnavailable: "暗号化設定のインポートはこのブラウザ環境では利用できません。HTTPS、localhost、またはPanDB Desktopを使用してください。",
  ...
  ai: {
    ...
    codexMcpRequiredTitle: "PanDB MCP Serverが必要です",
    codexMcpRequiredDescription: "Codex CLIはPanDB MCP Server経由でデータベーススキーマとクエリツールにアクセスします。AIアシスタントでCodexを使う前にインストールしてください。",
    ...
    codexCliEnvReservedName: "{name}はMCP Server用にPanDBが管理しているため、ここでは設定できません。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "PanDBが管理するMCP環境変数が上書きされています。プロバイダー設定からDBX_MCP_*変数を削除してください。",
      claudeCodeNotAuthenticated: "Claude Code CLIにログインしていません。ターミナルで`claude auth login`を実行してから再試行してください。",
      claudeCodeMcpConfigInvalid: "Claude CodeがPanDBのMCP設定を読み取りまたは解析できません。Claude Code CLIのバージョンと実行ファイルのパスを確認してください。",
      dbxMcpMissing: "互換性のあるPanDB MCP Serverが見つかりません。設定 > MCPで確認し、再インストールまたは更新してください。",
      claudeCodeMcpStartupFailed: "Claude CodeはMCP設定を読み込みましたが、PanDB MCP Serverを起動できません。設定 > MCPと以下の診断詳細を確認してください。",
      claudeCodeCommandLineTooLong: "Windowsが長すぎるClaude Codeコマンドを拒否しました。PanDBを更新し、生成されたMCP設定ファイルで再試行してください。",
      ...
      piAgentEnvReserved: "PanDBが管理するPiまたはMCP環境変数が上書きされています。DBX_PI_*とDBX_MCP_*を削除してください。",
      piAgentNotAuthenticated: "Pi Coding Agentに認証済みのモデルプロバイダーがありません。Piでプロバイダーを設定またはログインしてください。",
      piAgentMcpStartupFailed: "PiはPanDB MCPブリッジを起動できませんでした。設定 > MCPと以下の診断詳細を確認してください。",
  ...
  contextMenu: {
    ...
    parameterDefaultHint: "DEFAULTがチェックされている場合、PanDBはその引数を省略し、データベースがプロシージャのデフォルト値を使用します。",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "クラスタ状態は引き続き利用できます。PanDB は現在の etcd エンドポイントへ直接 /metrics を要求するため、生成された URL に PanDB アプリケーションから到達できることを確認してください。",
  ...
  sqlParameters: {
    title: "SQLパラメーター",
    description: "SQLテンプレートのプレースホルダーに値を入力します。PanDBは実行前にSQLリテラルへ置換します。",
    name: "パラメーター",
    syntax: "Syntax",
    type: "型",
    value: "値",
  ...
  settings: {
    ...
    iconThemeDefaultDescription: "カラーPanDBアプリアイコンを使用。",
    iconThemeBlack: "黒",
    iconThemeBlackDescription: "アプリ内で黒いPanDBロゴを使用。",
    iconThemeBlackDescriptionMac: "アプリ内は黒いロゴ、Dockは黒い背景、メニューバーはシステムに従います。",
    showTrayIcon: "トレイ/メニューバーアイコンを表示",
    showTrayIconDescription: "無効時はアイコンが表示されませんが、ウィンドウを閉じると従来通りPanDBはバックグラウンドに隠れます。",
    quitOnClose: "ウィンドウを閉じるときに終了",
    quitOnCloseDescription: "有効にすると、閉じるボタンでトレイに隠すのではなく、PanDBを完全に終了します。",
    closeActionPromptTitle: "ウィンドウを閉じる",
    closeActionPromptDescription: "閉じるボタンの動作を選択してください。後から「設定 → 外観」で変更できます。",
    closeActionQuit: "PanDBを終了",
    closeActionMinimize: "トレイに最小化",
    updateNotificationsEnabled: "アップデート通知を有効にする",
    updateNotificationsEnabledDescription: "無効時、PanDBはアプリやドライバーのアップデートを自動確認せず、アップデートバッジも表示しません。手動確認は引き続き利用可能です。",
    ...
    duckDbWorkerProcessIsolationDescription: "DuckDB クエリを独立した worker プロセスで実行します。長時間クエリがキャンセルに応答しない場合、PanDB は worker プロセスを停止してデータベースファイルのロックを解放できます。",
    duckDbWorkerMaxProcesses: "DuckDB の最大同時 worker 数",
    duckDbWorkerMaxProcessesDescription: "PanDB が同時に実行できる DuckDB worker プロセス数を制限します。範囲: 1-16。既定値: 4。",
    duckDbWorkerProcessIsolationRestartRequired: "この変更を有効にするには PanDB を再起動してください。",
    restartDbx: "PanDB を再起動",
    restartDbxFailed: "PanDB の再起動に失敗しました: {error}",
    ...
    openTabsRestoreModeDescription: "PanDB 起動時に前回開いていたタブをどのように復元するかを選択します。",
    ...
    syncWebDavDescription: "保存済みSQLライブラリを含むPanDBスナップショットをWebDAV互換ストレージへアップロードまたは復元します。",
    ...
    syncRemotePathDescription: "アップロード時に不足している親フォルダをPanDBが作成します。",
    syncSecretNotice: "デフォルトでは、PanDBは接続詳細と設定のみを同期します。データベースパスワード、SSHパスワード、プロキシパスワード、接続文字列、AI APIキーはローカルに保持されます。",
    syncSecrets: "暗号化されたシークレットを同期",
    syncSecretsDescription: "有効時、PanDBはデータベースパスワード、SSHパスワード、AI APIキーを暗号化してからWebDAVにアップロードします。",
    syncSecretsPassphrase: "同期パスワード",
    syncSecretsPassphraseDescription: "このパスワードは機密データの暗号化と復元にのみ使用されます。PanDBは保存せず、各デバイスで再度入力する必要があります。",
    syncTest: "テスト",
    syncUpload: "アップロード",
    syncDownload: "ダウンロード",
    ...
    syncAutoUploadDescription: "PanDBを開いている間だけ実行されます。ローカルの保存済みSQLを誤って置き換えないよう、ダウンロードは手動確認が必要です。",
    ...
    syncDownloadConfirm: "リモートPanDBスナップショットをダウンロードして適用しますか？ローカルのメタデータと保存済みSQLが置き換えられます。",
    apply: "適用",
    ...
    mcpTitle: "MCPサーバー",
    mcpDescription: "Claude Code、Cursor、その他のエージェントが使用するPanDB MCPサーバーのインストールとバージョンを確認します。",
    mcpChecking: "確認中",
    ...
    mcpConfig: "MCP設定",
    mcpConfigOptionsHint: "接続範囲と実行権限はPanDBがすべてのMCPクライアントに一元適用します。生成される設定にはMCP Serverの起動と現在のPanDBへの接続に必要な実行パラメーターのみが含まれ、権限や接続範囲の変数は含まれません。",
    mcpScopeConnection: "MCPに許可する接続",
    mcpScopeConnectionDescription: "PanDBは選択した接続だけをMCPに公開します。変更はすべてのMCPクライアントに適用されます。",
    ...
    mcpExecutionModeDescription: "PanDBがすべてのMCPクライアントに適用する権限レベルを選択します。クライアント側の読み書き権限設定では、このレベルを変更できません。",
    ...
    mcpExecutionModeReadOnlyDescription: "PanDBが読み取りと判定したリクエストのみを許可し、識別できるデータベース書き込みと接続変更を拒否します。データベースアカウントの権限が最終的な安全境界です。",
    ...
    mcpCapabilityDescription: "代表的な操作を示しています。PanDBが確実に分類できないリクエストにはフルアクセスが必要です。",
    ...
    mcpDetectionTiming: "このページを開くとPanDBが自動的に確認します。更新するには再確認を使用してください。",
    mcpNpmBoundary: "PanDBはMCPステータスの確認と説明のみを行います。インストールとアップグレードはnpmを通じて実行されます。",
    mcpRefresh: "再確認",
    mcpGuide: "MCPガイド",
    ...
    supportInfoAppVersion: "PanDBバージョン",
    ...
    syncSnippetDescription: "同一の PanDB スナップショットを非公開スニペットに保存します。初回アップロード時にスニペットが自動作成され、ID が保存されます。",
    ...
    syncSnippetMigrateLegacyConfirm: "PanDB はまず暗号化された置換スニペットを作成し、その後、旧式の平文スニペット {id} を完全に削除します。この操作は取り消せません。続行しますか？",
  ...
  nacos: {
    ...
    persistentInstanceHint: "PanDB はハートビートを維持できないため、一時インスタンスを登録しません。",
    ipAddress: "IP アドレス",
    port: "ポート",
  ...
  databaseBackup: {
    ...
    runtimeRequirement: "PanDBが開いている状態（トレイに最小化したときも含む）で、定期バックアップが実行されます。",
