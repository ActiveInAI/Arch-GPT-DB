...
export default withEnglishFallback({
  ...
  externalSqlFile: {
    ...
    deletedTitle: "文件已不存在",
    deletedDescription: "{path} 已在 PanDB 外部被删除或移动，编辑器中的内容仍然保留。",
  ...
  updates: {
    ...
    availableMessage: "PanDB {latest} 已发布，当前版本为 {current}。",
    upToDate: "PanDB 已是最新版本 ({version})。",
    failed: "检查更新失败：{error}",
    rateLimited: "GitHub 更新检查暂时触发频率限制。你仍然可以打开下载页手动查看。",
    openRelease: "打开下载页",
    downloadAndInstall: "下载并安装",
    activeTasksBlockUpdate: "有 {count} 个任务正在执行，请等待任务完成后再更新 PanDB。",
    portableAutomaticUpdate: "PanDB 将下载已签名的便携版 ZIP，退出后仅替换 PanDB.exe 并自动重启。portable.pandb 和 data 会保留。",
  ...
  connection: {
    ...
    oracleTnsPathHint: "该路径由 PanDB 后端/Oracle Agent 读取。桌面端填写本机路径，Web 和 Docker 部署填写服务器路径。",
    ...
    kafkaKerberosKeytabPlaceholder: "PanDB Agent 所在机器路径，例如 /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "可选，PanDB Agent 所在机器路径，例如 /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab 和 krb5.conf 路径由 PanDB Agent 读取，必须存在于运行 PanDB Agent 的机器上；不会从当前浏览器上传文件。",
    ...
    gaussdbConnectionModeNativeHint: "使用 PanDB 内置的 PostgreSQL 兼容协议，适用于 GaussDB 的 A/PG/Oracle 兼容模式。",
    ...
    agentJavaTooOld: "该驱动需要 Java 21。请在驱动管理器中使用 PanDB 托管 JRE 21，或选择 Java 21 可执行文件。",
    ...
    jdbcDocs: "查看 JDBC 文档",
    jdbcPluginHint: "先安装 PanDB JDBC 插件。有外部依赖的驱动请从驱动管理使用 Maven 安装，或同时导入驱动及全部依赖 JAR。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」有未保存的 SQL 更改，退出 PanDB 后将丢失。是否保存后再退出？",
    unsavedChangesAppCloseMultipleMessage: "当前有 {count} 个 SQL 标签未保存，当前已定位到「{title}」。退出 PanDB 后将丢失，是否保存后再退出？",
  ...
  tabs: {
    ...
    externalFileMissing: "已在 PanDB 外部删除或移动",
  ...
  welcome: {
    title: "数据库工作台",
    subtitle: "选择左侧连接开始浏览结构，或直接创建查询标签页。PanDB 支持多种数据库、查询历史、数据表格编辑和 AI SQL 助手。",
    connections: "连接",
    connected: "已连接",
    databaseTypes: "数据库类型",
  ...
  configExport: {
    ...
    cryptoUnavailable: "当前浏览器环境不支持加密配置导入，请使用 HTTPS、localhost 或 PanDB 桌面版。",
  ...
  ai: {
    ...
    codexCliEnvReservedName: "{name} 由 PanDB 用于 MCP Server，不能在这里设置。",
    cliMcpRequiredTitle: "需要 PanDB MCP Server",
    cliMcpRequiredDescription: "{provider} 会通过 PanDB MCP Server 访问数据库结构和查询工具。请先安装后再在 AI 助手中使用该提供商。",
    cliPath: "{provider} 路径",
    ...
    cliEnvReservedName: "{name} 由 PanDB 用于 MCP Server，不能在这里设置。",
    codexMcpRequiredTitle: "需要 PanDB MCP Server",
    codexMcpRequiredDescription: "Codex CLI 通过 PanDB MCP Server 访问数据库结构和查询工具。请先安装后再在 AI 助手中使用 Codex。",
    reasoningLevel: "推理级别",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "配置覆盖了由 PanDB 管理的 MCP 环境变量。请从供应商配置中移除 DBX_MCP_* 变量。",
      claudeCodeNotAuthenticated: "Claude Code CLI 尚未登录。请在终端执行 `claude auth login` 后重试。",
      claudeCodeMcpConfigInvalid: "Claude Code 无法读取或解析 PanDB 生成的 MCP 配置。请检查 Claude Code CLI 版本和可执行文件路径。",
      dbxMcpMissing: "未找到兼容的 PanDB MCP Server。请在 设置 > MCP 中检查并重新安装或更新。",
      claudeCodeMcpStartupFailed: "Claude Code 已加载 MCP 配置，但无法启动 PanDB MCP Server。请检查 设置 > MCP 和下方诊断详情。",
      claudeCodeCommandLineTooLong: "Windows 拒绝了过长的 Claude Code 命令。请更新 PanDB，并使用生成的 MCP 配置文件重试。",
      ...
      piAgentEnvReserved: "配置覆盖了由 PanDB 管理的 Pi 或 MCP 环境变量。请移除 DBX_PI_* 和 DBX_MCP_* 变量。",
      piAgentNotAuthenticated: "Pi Coding Agent 没有可用的已认证模型供应商。请先在 Pi 中配置或登录供应商。",
      piAgentMcpStartupFailed: "Pi 无法启动 PanDB 的作用域 MCP 桥接。请检查 设置 > MCP 和下方诊断详情。",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "集群状态仍可正常查看。PanDB 会直接访问当前 etcd endpoint 拼接后的 /metrics，请确认该地址可由 PanDB 应用访问。",
  ...
  sqlParameters: {
    title: "SQL 参数",
    description: "为 SQL 模板占位符填写参数值，PanDB 会在执行前替换为 SQL 字面量。",
    name: "参数名",
    syntax: "Syntax",
    type: "类型",
    value: "参数值",
  ...
  settings: {
    ...
    iconThemeDefault: "默认",
    iconThemeDefaultDescription: "使用彩色 PanDB 应用图标。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "应用内使用黑色 PanDB 标识。",
    ...
    quitOnCloseDescription: "开启后，点击窗口关闭按钮将彻底退出 PanDB，而不是隐藏到系统托盘。",
    ...
    updateNotificationsEnabledDescription: "关闭后，PanDB 不会自动检查应用和驱动更新，也不会显示更新红点；仍可手动检查更新。",
    ...
    duckDbWorkerProcessIsolationDescription: "将 DuckDB 查询放到独立 worker 进程中运行。停止长查询时，如果 DuckDB 未及时响应取消请求，PanDB 会结束 worker 进程以释放数据库文件锁。",
    duckDbWorkerMaxProcesses: "DuckDB 最大并发 worker 数",
    duckDbWorkerMaxProcessesDescription: "限制 PanDB 同时运行的 DuckDB worker 进程数量。范围：1-16，默认：4。",
    duckDbWorkerProcessIsolationRestartRequired: "重启 PanDB 后生效。",
    restartDbx: "重启 PanDB",
    restartDbxFailed: "重启 PanDB 失败：{error}",
    debugLogsCopy: "复制日志",
    debugLogsCopied: "已复制",
    ...
    openTabsRestoreModeDescription: "选择 PanDB 启动时如何恢复上次打开的标签页。",
    ...
    syncWebDavDescription: "将包含 SQL 库的 PanDB 快照上传到兼容 WebDAV 的存储服务，或从远端恢复。",
    ...
    syncAutoUploadDescription: "仅在 PanDB 打开时运行。下载仍需手动确认，避免意外覆盖本地保存的 SQL。",
    ...
    syncDownloadConfirm: "要下载并应用远端 PanDB 快照吗？本地元信息和保存的 SQL 会被替换。",
    syncSnippetTitle: "GitHub / Gitee 代码片段同步",
    syncSnippetDescription: "将同一份 PanDB 快照保存到私有代码片段。首次上传会自动创建代码片段并保存其 ID。",
    ...
    syncSnippetMigrateLegacyConfirm: "PanDB 会先创建新的加密代码片段，再永久删除旧明文代码片段 {id}。此操作无法撤销，是否继续？",
    ...
    mcpTitle: "MCP Server",
    mcpDescription: "检查 Claude Code、Cursor 等编程助手使用的 PanDB MCP Server 安装与版本状态。",
    mcpChecking: "检查中",
    ...
    mcpConfig: "MCP 配置",
    mcpConfigOptionsHint: "连接范围和执行权限均由 PanDB 统一控制并对所有 MCP 客户端立即生效；生成配置只包含启动 MCP Server 和连接当前 PanDB 所需的运行参数，不包含权限或连接范围变量。",
    mcpScopeConnection: "允许 MCP 访问的连接",
    mcpScopeConnectionDescription: "PanDB 只向 MCP 暴露选中的连接，修改后对所有 MCP 客户端生效。",
    mcpScopeMode: "连接范围模式",
    ...
    mcpExecutionModeDescription: "选择 PanDB 对所有 MCP 客户端强制执行的权限级别。客户端的读写权限配置无法更改此级别。",
    ...
    mcpExecutionModeReadOnlyDescription: "仅允许 PanDB 判定为读取的请求；禁止已识别的数据库写入和连接管理变更。数据库账号权限仍是最终硬边界。",
    ...
    mcpNpmBoundary: "PanDB 只检测和提示 MCP 状态；安装与升级仍由 npm 完成。",
    mcpRefresh: "重新检查",
    mcpGuide: "MCP 指南",
    ...
    supportInfoAppVersion: "PanDB 版本",
  ...
  driverStore: {
    ...
    javaRuntimeManaged: "PanDB 托管 JRE",
  ...
  databaseBackup: {
    title: "数据库备份",
    schedules: "备份计划",
    runtimeRequirement: "PanDB 处于打开状态时会执行定时备份，包括最小化到托盘时。",
    addSchedule: "新建计划",
    editSchedule: "编辑计划",
  ...
  nacos: {
    ...
    persistentInstanceHint: "PanDB 不注册临时实例，因为无法为其提供心跳维持。",
    ipAddress: "IP 地址",
    port: "端口",
