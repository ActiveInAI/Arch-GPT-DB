...
export default withEnglishFallback({
  ...
  externalSqlFile: {
    modifiedTitle: "檔案已在 PanDB 外部變更",
    modifiedDescription: "{path} 已被其他程式修改，請選擇要保留的版本。",
    deletedTitle: "檔案已不存在",
    deletedDescription: "{path} 已在 PanDB 外部刪除或移動，編輯器中的內容仍然保留。",
  ...
  updates: {
    ...
    availableMessage: "PanDB {latest} 已發布，目前版本為 {current}。",
    upToDate: "PanDB 已是最新版本 ({version})。",
    failed: "檢查更新失敗：{error}",
    rateLimited: "GitHub 更新檢查暫時觸發頻率限制。你仍然可以開啟下載頁手動檢視。",
    openRelease: "開啟下載頁",
    downloadAndInstall: "下載並安裝",
    activeTasksBlockUpdate: "有 {count} 個任務正在執行，請等待任務完成後再更新 PanDB。",
    portableAutomaticUpdate: "PanDB 將下載已簽署的可攜版 ZIP，結束後僅替換 PanDB.exe 並自動重新啟動。portable.pandb 和 data 會保留。",
  ...
  connection: {
    ...
    oracleTnsPathHint: "此路徑由 PanDB 後端/Oracle Agent 讀取。桌面版填寫本機路徑，Web 和 Docker 部署填寫伺服器路徑。",
    ...
    gaussdbConnectionModeNativeHint: "使用 PanDB 內建的 PostgreSQL 相容協定，適用於 GaussDB 的 A/PG/Oracle 相容模式。",
    ...
    agentJavaTooOld: "此驅動程式需要 Java 21。請在驅動程式管理器中使用 PanDB 託管 JRE 21，或選擇 Java 21 可執行檔。",
    ...
    jdbcDocs: "檢視 JDBC 文件",
    jdbcPluginHint: "先安裝 PanDB JDBC 外掛程式，再匯入資料庫廠商提供的 JDBC 驅動程式 JAR。",
    ...
    kafkaKerberosKeytabPlaceholder: "PanDB Agent 所在機器路徑，例如 /etc/security/keytabs/user.keytab",
    kafkaKerberosKrb5ConfPlaceholder: "可選，PanDB Agent 所在機器路徑，例如 /etc/krb5.conf",
    kafkaKerberosPathHint: "keytab 和 krb5.conf 路徑由 PanDB Agent 讀取，必須存在於執行 PanDB Agent 的機器上；不會從當前瀏覽器上傳檔案。",
  ...
  editor: {
    ...
    unsavedChangesAppCloseMessage: "「{title}」有未儲存的 SQL 變更，結束 PanDB 後將遺失。儲存後再結束嗎？",
    unsavedChangesAppCloseMultipleMessage: "目前有 {count} 個 SQL 分頁未儲存，目前已定位到「{title}」。結束 PanDB 後將遺失，儲存後再結束嗎？",
  ...
  tabs: {
    ...
    externalFileMissing: "已在 PanDB 外部刪除或移動",
  ...
  grid: {
    ...
    queryEditUnsupported: {
      ...
      "complex-source": "PanDB 無法為此結果唯一確定一個安全的回寫目標資料表。",
      ...
      "metadata-unavailable": "PanDB 無法載入資料表 metadata，因此已停用結果編輯。",
    },
  ...
  configExport: {
    ...
    cryptoUnavailable: "目前瀏覽器環境不支援加密設定匯入，請使用 HTTPS、localhost 或 PanDB 桌面版。",
  ...
  ai: {
    ...
    codexMcpRequiredDescription: "Codex CLI 會透過 PanDB MCP Server 存取資料庫結構與查詢工具。請先安裝後再於 AI 助理中使用 Codex。",
    ...
    codexCliEnvReservedName: "{name} 由 PanDB 用於 MCP Server，不能在這裡設定。",
    ...
    cliErrors: {
      ...
      claudeCodeEnvReserved: "設定覆寫了由 PanDB 管理的 MCP 環境變數。請從供應商設定中移除 DBX_MCP_* 變數。",
      claudeCodeNotAuthenticated: "Claude Code CLI 尚未登入。請在終端機執行 `claude auth login` 後重試。",
      claudeCodeMcpConfigInvalid: "Claude Code 無法讀取或解析 PanDB 產生的 MCP 設定。請檢查 Claude Code CLI 版本和可執行檔路徑。",
      dbxMcpMissing: "找不到相容的 PanDB MCP Server。請在 設定 > MCP 中檢查並重新安裝或更新。",
      claudeCodeMcpStartupFailed: "Claude Code 已載入 MCP 設定，但無法啟動 PanDB MCP Server。請檢查 設定 > MCP 和下方診斷詳情。",
      claudeCodeCommandLineTooLong: "Windows 拒絕了過長的 Claude Code 命令。請更新 PanDB，並使用產生的 MCP 設定檔重試。",
      ...
      piAgentEnvReserved: "設定覆寫了由 PanDB 管理的 Pi 或 MCP 環境變數。請移除 DBX_PI_* 與 DBX_MCP_* 變數。",
      piAgentNotAuthenticated: "Pi Coding Agent 沒有可用的已驗證模型供應商。請先在 Pi 中設定或登入供應商。",
      piAgentMcpStartupFailed: "Pi 無法啟動 PanDB 的作用域 MCP 橋接。請檢查 設定 > MCP 和下方診斷詳情。",
  ...
  sqlParameters: {
    title: "SQL 參數",
    description: "為 SQL 模板佔位符填寫參數值，PanDB 會在執行前替換為 SQL 字面量。",
    name: "參數名",
    syntax: "Syntax",
    type: "類型",
    value: "參數值",
  ...
  settings: {
    ...
    iconThemeDefault: "預設",
    iconThemeDefaultDescription: "使用彩色 PanDB 應用程式圖示。",
    iconThemeBlack: "黑色",
    iconThemeBlackDescription: "應用程式內使用黑色 PanDB 標誌。",
    ...
    quitOnCloseDescription: "開啟後，點擊視窗關閉按鈕將徹底退出 PanDB，而不是隱藏到系統匣。",
    ...
    updateNotificationsEnabledDescription: "關閉後，PanDB 不會自動檢查應用程式和驅動程式更新，也不會顯示更新紅點；仍可手動檢查更新。",
    ...
    duckDbWorkerProcessIsolationDescription: "將 DuckDB 查詢放到獨立 worker 進程中執行。停止長查詢時，如果 DuckDB 未及時回應取消請求，PanDB 會結束 worker 進程以釋放資料庫檔案鎖。",
    duckDbWorkerMaxProcesses: "DuckDB 最大並行 worker 數",
    duckDbWorkerMaxProcessesDescription: "限制 PanDB 同時執行的 DuckDB worker 進程數量。範圍：1-16，預設：4。",
    duckDbWorkerProcessIsolationRestartRequired: "重啟 PanDB 後生效。",
    restartDbx: "重啟 PanDB",
    restartDbxFailed: "重啟 PanDB 失敗：{error}",
    debugLogsCopy: "複製日誌",
    debugLogsCopied: "已複製",
    ...
    openTabsRestoreModeDescription: "選擇 PanDB 啟動時如何還原上次開啟的分頁。",
    ...
    syncWebDavDescription: "上傳包含 SQL 庫的 PanDB 快照到相容 WebDAV 的儲存服務，或從該服務還原快照。",
    ...
    syncRemotePathDescription: "上傳時 PanDB 會自動建立缺失的父目錄。",
    syncSecretNotice: "預設只同步連線詳細資料和設定。資料庫密碼、SSH 密碼、代理伺服器密碼、連線字串和 AI API Key 會保留在本機。",
    syncSecrets: "同步加密後的敏感資訊",
    syncSecretsDescription: "開啟後，PanDB 會先加密資料庫密碼、SSH 密碼和 AI API Key，再上傳到 WebDAV。",
    syncSecretsPassphrase: "同步密碼",
    syncSecretsPassphraseDescription: "此密碼只用於加密和還原敏感資料。PanDB 不會儲存；每台裝置都需要再次輸入。",
    syncTest: "測試",
    syncUpload: "上傳",
    syncDownload: "下載",
    ...
    syncAutoUploadDescription: "僅在 PanDB 開啟時執行。下載仍需手動確認，避免意外覆蓋本機已儲存的 SQL。",
    ...
    syncDownloadConfirm: "要下載並套用遠端 PanDB 快照嗎？本機 metadata 和已儲存的 SQL 會被取代。",
    apply: "套用",
    applyAndClose: "套用並關閉",
    reset: "重設",
    resetDefaults: "還原預設",
    ...
    mcpTab: "MCP",
    mcpTitle: "MCP Server",
    mcpDescription: "檢查 Claude Code、Cursor 等程式助理使用的 PanDB MCP Server 安裝與版本狀態。",
    mcpChecking: "檢查中",
    ...
    mcpConfig: "MCP 設定",
    mcpConfigOptionsHint: "連線範圍與執行權限均由 PanDB 統一控制，並對所有 MCP 用戶端立即生效；產生的設定只包含啟動 MCP Server 與連線目前 PanDB 所需的執行參數，不包含權限或連線範圍變數。",
    mcpScopeConnection: "允許 MCP 存取的連線",
    mcpScopeConnectionDescription: "PanDB 只向 MCP 公開選取的連線，變更後對所有 MCP 用戶端生效。",
    mcpScopeMode: "連線範圍模式",
    ...
    mcpExecutionModeDescription: "選擇 PanDB 對所有 MCP 用戶端強制執行的權限等級。用戶端的讀寫權限設定無法變更此等級。",
    ...
    mcpExecutionModeReadOnlyDescription: "僅允許 PanDB 判定為讀取的請求；禁止已識別的資料庫寫入與連線管理變更。資料庫帳號權限仍是最終硬邊界。",
    ...
    mcpNpmBoundary: "PanDB 只檢測和提示 MCP 狀態；安裝與升級仍由 npm 完成。",
    mcpRefresh: "重新檢查",
    mcpGuide: "MCP 指南",
    ...
    syncSnippetMigrateLegacyConfirm: "PanDB 會先建立新的加密程式碼片段，再永久刪除舊明文程式碼片段 {id}。此操作無法復原，是否繼續？",
  ...
  driverStore: {
    ...
    javaRuntimeManaged: "PanDB 託管 JRE",
  ...
  nacos: {
    ...
    persistentInstanceHint: "PanDB 不註冊臨時實例，因為無法維持其心跳。",
    ipAddress: "IP 位址",
    port: "連接埠",
  ...
  etcd: {
    ...
    dashboard: {
      ...
      metricsUnavailableHint: "叢集狀態仍可正常檢視。PanDB 會直接存取目前 etcd endpoint 拼接後的 /metrics，請確認 PanDB 應用程式可以存取該位址。",
  ...
  databaseBackup: {
    title: "資料庫備份",
    schedules: "備份計畫",
    runtimeRequirement: "PanDB 處於開啟狀態時會執行定時備份，包含最小化到托盤時。",
    addSchedule: "新建計畫",
    editSchedule: "編輯計畫",
