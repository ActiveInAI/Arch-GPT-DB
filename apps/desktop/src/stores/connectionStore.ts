...
export const useConnectionStore = defineStore("connection", () => {
  ...
  const completionMetadataLimiter = new MetadataTaskLimiter(COMPLETION_METADATA_CONCURRENCY, (event) => {
    console.debug("[PanDB][completion-metadata:limit]", event);
  });
  ...
  const metadataLoadCoordinator = new MetadataLoadCoordinator((event) => {
    console.debug("[PanDB][metadata-load:coordinator]", event);
  });
  ...
  const metadataTraceLogger: MetadataLoadTraceLogger = (event) => {
    console.debug("[PanDB][metadata-load:trace]", event);
  };
  ...
  function trackDisconnectRequest(connectionId: string, request: Promise<void>, scoped: boolean): Promise<void> {
    ...
    const tracked = bounded
      .catch((error) => {
        console.warn("[PanDB][connection:disconnect-error]", { connectionId, error });
      })
      .finally(() => {
  ...
  function startCancelDisconnectRequest(connectionId: string, attempt: number): Promise<void> {
    ...
    const tracked = withDisconnectRequestTimeout(connectionId, request)
      .catch((error) => {
        console.warn("[PanDB][connection:cancel-disconnect-error]", { connectionId, attempt, error });
        throw error;
      })
      .finally(() => {
  ...
  async function cleanupResolvedCancelledConnectionAttempt(connectionId: string, attempt: number) {
    try {
      ...
      console.warn("[PanDB][connection:cancel-result-cleanup-error]", { connectionId, attempt, error });
    }
  }
  ...
  async function withDisconnectRequestTimeout(connectionId: string, promise: Promise<void>): Promise<void> {
    ...
    void promise.catch((error) => {
      if (timedOut) console.warn("[PanDB][connection:disconnect-late-error]", { connectionId, error });
    });
    try {
      await Promise.race([
        promise,
        new Promise<void>((resolve) => {
          timer = setTimeout(() => {
            timedOut = true;
            console.warn("[PanDB][connection:disconnect-timeout]", { connectionId, timeoutMs: DISCONNECT_REQUEST_TIMEOUT_MS });
            resolve();
    ...
    void promise.then(
      (connectionId) => {
        ...
        void api.disconnectDb(cleanupConnectionId).catch((error) => {
          console.warn("[PanDB][connection:timeout-cleanup-failed]", { connectionId: cleanupConnectionId, error });
        });
      },
      (error) => {
        if (!timedOut) return;
  ...
  async function loadSimpleSupplementalObjectChildren(options: {
    ...
    try {
      ...
      console.debug("[PanDB][metadata:simple-supplemental:error]", {
        connectionId: options.connectionId,
        database: options.database,
        schema: options.effectiveSchema,
        error,
  ...
  async function loadSidebarDatabaseStorage(connectionId: string, options?: { force?: boolean }): Promise<void> {
    ...
    try {
      ...
      console.debug("[PanDB][sidebar-database-storage:unavailable]", { connectionId, error });
    } finally {
  ...
  async function loadSidebarTableStorage(scope: SidebarTableStorageScope, options?: { force?: boolean }): Promise<void> {
    ...
    try {
      ...
      console.debug("[PanDB][sidebar-table-storage:unavailable]", { ...scope, error });
    } finally {
  ...
  async function readImportFile(source: ImportSource = "dbx"): Promise<{ content: string; encrypted: boolean } | null> {
    ...
    if (isTauriRuntime()) {
      ...
      const { readTextFile } = await import("@tauri-apps/plugin-fs");
      const path = await open({
        filters: source === "navicat" ? [{ name: "Navicat Connection Export", extensions: ["ncx", "xml"] }] : [{ name: "PanDB JSON", extensions: ["json"] }],
        multiple: false,
      });
      if (!path) return null;
