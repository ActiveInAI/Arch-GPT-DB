...
async function applyUiScale(scale: number) {
  ...
  try {
    ...
    console.warn("[Arch-GPT DB] Failed to apply UI scale", { scale, error });
  }
}
...
function openGitHub() {
  openUrl("https://github.com/ActiveInAI/Arch-GPT-DB");
}
function openMcpGuide() {
  openUrl("https://github.com/ActiveInAI/Arch-GPT-DB/tree/main/docs/content/docs");
}
...
function routeAiRedisCommand(command: string, execute: boolean): boolean {
  ...
  if (!routed) {
    console.warn("[Arch-GPT DB] Redis AI command could not reach the active Redis console");
    return true;
  }
  void routed.then((handled) => {
    if (!handled) console.warn("[Arch-GPT DB] Redis AI command could not reach the active Redis console");
  });
  return true;
}
