...
async function applyUiScale(scale: number) {
  ...
  try {
    ...
    console.warn("[PanDB] Failed to apply UI scale", { scale, error });
  }
}
...
function openGitHub() {
  openUrl("https://github.com/ActiveInAI/PanDB");
}
function openMcpGuide() {
  openUrl("https://github.com/ActiveInAI/PanDB/tree/main/docs/content/docs");
}
...
function routeAiRedisCommand(command: string, execute: boolean): boolean {
  ...
  if (!routed) {
    console.warn("[PanDB] Redis AI command could not reach the active Redis console");
    return true;
  }
  void routed.then((handled) => {
    if (!handled) console.warn("[PanDB] Redis AI command could not reach the active Redis console");
  });
  return true;
}
