...
export function parseMongoCommand(input: string): ParsedMongoCommand | null {
  ...
  const parsers: Array<(source: string) => MongoCommand | null> = [
    ...
    (source) => {
      ...
      // while mapping to PanDB's countDocuments-compatible result path.
...
export function evaluateMongoWriteSafety(command: MongoWriteCommand, options: MongoAggregateSafetyOptions): { allowed: boolean; reason?: string } {
  if (!options.allowWrites) {
    return {
      allowed: false,
      reason: "MCP MongoDB execution is read-only under the current PanDB policy.",
    };
  }
  ...
  if (!options.allowDangerous && highRisk) {
    return {
      allowed: false,
      reason: `MongoDB ${command.kind} requires high-risk operations to be enabled in PanDB MCP settings.`,
    };
  }
  return { allowed: true };
}
...
export function evaluateMongoAggregateSafety(command: MongoAggregateCommand, options: MongoAggregateSafetyOptions): { allowed: boolean; reason?: string } {
  ...
  if (!options.allowWrites) {
    return {
      allowed: false,
      reason: `MongoDB aggregate stage "${writeStage}" is blocked by the current PanDB MCP read-only policy.`,
    };
  }
  if (!options.allowDangerous) {
    return {
      allowed: false,
      reason: `MongoDB aggregate stage "${writeStage}" requires high-risk operations to be enabled in PanDB MCP settings.`,
    };
  }
  return { allowed: true };
}
