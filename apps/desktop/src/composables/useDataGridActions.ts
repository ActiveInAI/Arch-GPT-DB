...
export function useDataGridActions(activeTab: ComputedRef<QueryTab | undefined>) {
  ...
  async function refreshDataTabTableMeta(tab: QueryTab, trace?: { traceId: string; elapsed: () => string }): Promise<void> {
    ...
    console.info("[PanDB][reloadData:metadata:ensure-connected:start]", { traceId: trace?.traceId, elapsed: trace?.elapsed() });
    await connectionStore.ensureConnected(target.connectionId);
    console.info("[PanDB][reloadData:metadata:ensure-connected:done]", { traceId: trace?.traceId, elapsed: trace?.elapsed() });
    const config = connectionStore.getConfig(target.connectionId);
    const querySchema = metadataSchemaForConnection(config, target.database, target.schema);
    console.info("[PanDB][reloadData:metadata:get-columns:start]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), schema: querySchema, table: target.tableName });
    ...
    console.info("[PanDB][reloadData:metadata:get-columns:done]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), columnCount: columns.length });
    ...
    if (!current || current.mode !== "data" || current.connectionId !== target.connectionId || current.database !== target.database || currentMeta?.tableName !== target.tableName || (currentMeta.schema ?? "") !== (target.schema ?? "") || (currentMeta.catalog ?? "") !== (target.catalog ?? "")) {
      console.info("[PanDB][reloadData:metadata:stale-tab]", { traceId: trace?.traceId, elapsed: trace?.elapsed(), table: target.tableName });
      return;
    }
  ...
  async function onReloadData(sql?: string, _searchText?: string, whereInput?: string, orderBy?: string, limit?: number, offset?: number, intent?: DataGridReloadIntent) {
    ...
    if (tab.mode === "data" && tableMetaForDataTab(tab)) {
      ...
      console.info("[PanDB][reloadData:start]", {
        traceId,
        tabId: tab.id,
        connectionId: tab.connectionId,
        database: tab.database,
      ...
      const startMetadataRefresh = () => {
        console.info("[PanDB][reloadData:metadata:background:start]", { traceId, elapsed: elapsed(), reason: hasRealTableMetaColumns ? "stale" : "missing", metadataAgeMs });
        void refreshDataTabTableMeta(tab, { traceId, elapsed })
          .then(() => {
            console.info("[PanDB][reloadData:metadata:background:done]", { traceId, elapsed: elapsed() });
          })
          .catch((e: any) => {
            console.warn("[PanDB][reloadData:metadata:background:error]", { traceId, elapsed: elapsed(), error: e });
      ...
      if (shouldRefreshMetadata) {
        ...
        console.info("[PanDB][reloadData:metadata:skip]", { traceId, elapsed: elapsed(), columnCount: tab.tableMeta!.columns.length, metadataAgeMs });
      }
      try {
        console.info("[PanDB][reloadData:build-sql:start]", { traceId, elapsed: elapsed() });
        ...
        console.info("[PanDB][reloadData:build-sql:done]", { traceId, elapsed: elapsed() });
        queryStore.updateSql(tab.id, nextSql);
        console.info("[PanDB][reloadData:execute:start]", { traceId, elapsed: elapsed() });
        await queryStore.executeTabSql(tab.id, nextSql, {
          pagination: { limit: pageLimit, offset: pageOffset },
          preserveResultDuringExecution: true,
        });
        console.info("[PanDB][reloadData:execute:done]", { traceId, elapsed: elapsed() });
      } catch (e) {
        console.error("[PanDB][reloadData:error]", { traceId, elapsed: elapsed(), error: e });
