...
export function resolveUpdateReleaseUrl(info: api.UpdateInfo | null, source: unknown, fallbackUrl: string): string {
  ...
  if (normalizedSource === "cnb" && info?.latest_version) {
    return `https://github.com/ActiveInAI/PanDB/releases/tag/${tagVersion(info.latest_version)}`;
  }
...
export function useAppUpdater(options: UseAppUpdaterOptions = {}) {
  ...
  const latestReleaseUrl = "https://github.com/ActiveInAI/PanDB/releases/latest";
  ...
  async function downloadAndInstallUpdate() {
    ...
    try {
      ...
      console.error("[PanDB update error]", rawMsg);
  ...
  async function installPendingUpdate() {
    ...
    try {
      ...
      // The portable helper exits and relaunches PanDB after the invoke response is delivered.
