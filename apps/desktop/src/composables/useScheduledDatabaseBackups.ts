...
export function useScheduledDatabaseBackups(options: { scheduler?: boolean } = {}) {
  ...
  async function pruneScheduleRuns(schedule: DatabaseBackupSchedule) {
    ...
    try {
      ...
      appendDebugLog("error", "[PanDB][database-backup:retention-error]", error);
    }
  }
      ...
      try {
        ...
        if (connection.db_type === "mysql" && schedule.tableFilterMode !== "all") {
          try {
            ...
            appendDebugLog("warn", "[PanDB][database-backup:table-name-case-detection-error]", error);
          }
        }
        ...
        if (finalStatus !== "success" && generatedPaths.length > 0) {
          try {
            ...
            appendDebugLog("error", "[PanDB][database-backup:partial-cleanup-error]", error);
        ...
        appendDebugLog(finalStatus === "success" ? "info" : "error", `[PanDB][database-backup:${finalStatus}]`, {
          scheduleId: schedule.id,
          runId,
    ...
    try {
      ...
      for (const schedule of dueSchedules) {
        await runSchedule(schedule.id, "scheduled").catch((error) => {
          appendDebugLog("error", "[PanDB][database-backup:scheduler-error]", error);
        });
      }
    } catch (error) {
      appendDebugLog("error", "[PanDB][database-backup:scheduler-init-error]", error);
    } finally {
