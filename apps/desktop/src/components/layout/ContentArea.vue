...
function loadDataGridComponent() {
  if (!dataGridComponentPromise) {
    dataGridComponentPromise = (async () => {
      ...
      if (shouldLogTiming) appendDebugLog("info", "[Arch-GPT DB][DataGrid:load:start]");
      const component = await import("@/components/grid/DataGrid.vue");
      if (shouldLogTiming) appendDebugLog("info", "[Arch-GPT DB][DataGrid:load:done]", { elapsed: `${Math.round(performance.now() - startedAt)}ms` });
      return component;
    })();
  }
...
watch(
  ...
  (result) => {
    ...
    appendDebugLog("info", "[Arch-GPT DB][ContentArea:result:observed]", {
      tabId: props.activeTab.id,
    ...
    nextTick(() => {
      appendDebugLog("info", "[Arch-GPT DB][ContentArea:result:nextTick]", {
        tabId: props.activeTab.id,
        elapsed: `${Math.round(performance.now() - startedAt)}ms`,
        isExecuting: props.activeTab.isExecuting,
      });
      requestAnimationFrame(() => {
        appendDebugLog("info", "[Arch-GPT DB][ContentArea:result:first-frame]", {
          tabId: props.activeTab.id,
          elapsed: `${Math.round(performance.now() - startedAt)}ms`,
          isExecuting: props.activeTab.isExecuting,
        });
...
async function onHandleClickColumn(matchedCols: Array<{ name: string; table: string; schema?: string }>, errorMsg?: string) {
  ...
  try {
    ...
    console.error("[Arch-GPT DB] Failed to fetch column info:", e);
    return;
  } finally {
