...
function loadDataGridComponent() {
  if (!dataGridComponentPromise) {
    dataGridComponentPromise = (async () => {
      ...
      if (shouldLogTiming) appendDebugLog("info", "[PanDB][DataGrid:load:start]");
      const component = await import("@/components/grid/DataGrid.vue");
      if (shouldLogTiming) appendDebugLog("info", "[PanDB][DataGrid:load:done]", { elapsed: `${Math.round(performance.now() - startedAt)}ms` });
      return component;
    })();
  }
...
watch(
  ...
  (result) => {
    ...
    appendDebugLog("info", "[PanDB][ContentArea:result:observed]", {
    ...
    nextTick(() => {
      appendDebugLog("info", "[PanDB][ContentArea:result:nextTick]", {
        tabId: props.activeTab.id,
        elapsed: `${Math.round(performance.now() - startedAt)}ms`,
        isExecuting: props.activeTab.isExecuting,
      });
      requestAnimationFrame(() => {
        appendDebugLog("info", "[PanDB][ContentArea:result:first-frame]", {
          tabId: props.activeTab.id,
          elapsed: `${Math.round(performance.now() - startedAt)}ms`,
          isExecuting: props.activeTab.isExecuting,
        });
...
async function onHandleClickColumn(matchedCols: Array<{ name: string; table: string; schema?: string }>, errorMsg?: string) {
  ...
  try {
    ...
    console.error("[PanDB] Failed to fetch column info:", e);
    return;
  } finally {
