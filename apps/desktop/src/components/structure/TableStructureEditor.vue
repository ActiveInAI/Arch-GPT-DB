...
async function hydrateRestoredDraftFromDatabase() {
  ...
  try {
    ...
    console.warn("[Arch-GPT DB][structure-editor:draft-hydration-failed]", e);
  } finally {
...
async function loadStructure(
  ...
  try {
    ...
    const secondaryMetadataPromise = applySecondaryMetadata()
      .catch((error) => {
        console.warn("[Arch-GPT DB][structure-editor:secondary-metadata-failed]", error);
      })
      .finally(() => {
    ...
    if (showErrors) {
      ...
      console.warn("[Arch-GPT DB][structure-editor:refresh-failed]", e);
    }
  } finally {
...
async function refreshStructureAfterSave(scope: TableStructureRefreshScope, characterLengthUnitsAfterSave: ReadonlyMap<string, string>) {
  try {
    ...
    console.warn("[Arch-GPT DB][structure-editor:post-save-refresh-failed]", e);
  } finally {
    postSaveRefreshing.value = false;
...
async function recordStructureHistory(sql: string, start: number, success: boolean, result?: { affected_rows?: number }, error?: string) {
  ...
  try {
    ...
    console.warn("[Arch-GPT DB][structure-history:save-failed]", e);
  }
}
