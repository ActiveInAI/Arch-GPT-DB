...
function exportFileFilter(format: EtcdExportFormat) {
  return format === "json" ? { name: "Arch-GPT DB etcd Bundle", extensions: ["json"] } : format === "csv" ? { name: "CSV Table", extensions: ["csv"] } : { name: "Markdown", extensions: ["md"] };
}
...
function validateBundle(input: unknown): EtcdBundle {
  ...
  if (candidate.format !== "dbx-etcd-bundle" || candidate.version !== 1 || !Array.isArray(candidate.entries)) {
    throw new Error("Unsupported etcd bundle. Expected Arch-GPT DB etcd bundle v1.");
  }
