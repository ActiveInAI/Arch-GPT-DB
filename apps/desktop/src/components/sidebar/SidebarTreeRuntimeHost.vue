...
async function loadTemplateContext(allowView = false) {
  ...
  try {
    ...
    console.warn("[PanDB][tableSqlTemplate:getColumns:error]", e);
  }
  ...
  if (dbType === "tdengine") {
    try {
      ...
      console.warn("[PanDB][tableSqlTemplate:listTables:error]", e);
    }
  }
...
async function refreshMutatedTableDataTabsForNode(node: TreeNode) {
  ...
  try {
    ...
    console.warn("[PanDB][table-data-refresh-after-mutation:error]", { target, error });
  }
}
