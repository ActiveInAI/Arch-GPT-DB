...
async function loadTemplateContext(allowView = false) {
  ...
  try {
    ...
    console.warn("[Arch-GPT DB][tableSqlTemplate:getColumns:error]", e);
  }
  ...
  if (dbType === "tdengine") {
    try {
      ...
      console.warn("[Arch-GPT DB][tableSqlTemplate:listTables:error]", e);
    }
  }
...
async function refreshMutatedTableDataTabsForNode(node: TreeNode) {
  ...
  try {
    ...
    console.warn("[Arch-GPT DB][table-data-refresh-after-mutation:error]", { target, error });
  }
}
