...
function vimModeExtension(enabled = settingsStore.editorSettings.vimModeEnabled) {
  ...
  // Beekeeper treats Vim as a first-class editor keymap. Keep it above PanDB's
...
async function ensureForeignKeysForTable(table: { name: string; database?: string | null; schema?: string | null }) {
  ...
  try {
    ...
    console.warn(`[PanDB] Failed to load foreign keys for ${cacheKey}:`, e);
...
async function resolveSqlHoverTooltip(currentView: EditorViewType, pos: number) {
  ...
  if (SEMANTIC_SQL_COMPLETION_ENABLED) {
    try {
      ...
      semanticModel = null;
      console.warn(`[PanDB] Failed to build semantic model for hover tooltip:`, error);
    }
  }
  ...
  try {
    ...
    if (table && !semanticQualifierIsRowSource && (!qualifier || table.schema?.toLowerCase() === qualifier.toLowerCase() || table.name === name)) {
      ...
      try {
        ...
        console.warn(`[PanDB] Failed to load table DDL for ${hoverDatabase}.${hoverSchema}.${table.name}:`, error);
      }
      ...
      if (!sqlContent) {
        ...
        try {
          ...
          console.warn(`[PanDB] Failed to load table metadata for ${hoverDatabase}.${hoverSchema}.${table.name}:`, error);
        }
        if (!metadataLoadFailed) {
          try {
            ...
            console.warn(`[PanDB] Failed to load table comment for ${hoverDatabase}.${hoverSchema}.${table.name}:`, error);
          }
        }
      ...
      return {
        ...
        create: () => ({
          dom: createHoverDom(table.name, sqlObjectHoverDetail(table), sqlContent, metadataLoadFailed ? ["[PanDB] Failed to load table structure — check connection"] : undefined),
        }),
      };
    }
    ...
    await Promise.all(
      refs.map(async (refTable) => {
        ...
        try {
          ...
          console.error(`[PanDB] Failed to load columns for ${cacheKey}:`, e);
        }
      }),
    );
  }
...
onMounted(async () => {
  ...
  const state = EditorState.create({
    ...
    extensions: [
      ...
      // Vim must be mounted before PanDB/default keymaps so normal-mode keys are handled first.
        ...
        mousedown: (event: MouseEvent) => {
          ...
          setTimeout(async () => {
            try {
              ...
              console.error("[PanDB] Ctrl+click error:", e);
            }
          }, 0);
          return true;
        },
