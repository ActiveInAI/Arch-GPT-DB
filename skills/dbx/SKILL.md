...
name: dbx
version: 1.0.0
description: "PanDB CLI for database schema exploration and read-only queries. When the user needs to list connections, explore tables, describe schemas, run queries, or generate AI-friendly schema context from PanDB-managed databases. Do NOT use for write operations unless the user explicitly confirms with --allow-writes."
metadata:
  requires:
    bins: ["dbx"]
  cliHelp: "dbx --help"
...
> **Prerequisite:** PanDB Desktop must be installed and configured with at least one connection. Run `dbx doctor` to verify setup. Install CLI: `npm install -g @dbx-app/cli`

## Core Concepts

- **Connection**: A named database connection configured in PanDB Desktop (e.g. "prod", "local"). Identified by name.
...
| `DBX_NOT_RUNNING` | Desktop bridge unavailable | Tell user to open PanDB Desktop. Check which commands work without bridge via `dbx capabilities`. |
...
PostgreSQL, MySQL (and compatible: Doris, StarRocks), SQLite run directly without PanDB Desktop. Other database types require the desktop bridge. Check with `dbx capabilities` to confirm.
...
4. **Missing desktop bridge** — If `dbx open` or bridge-required connections fail, run `dbx doctor` and tell the user to open PanDB Desktop. Commands that don't require the bridge: `connections list`, `schema list`, `schema describe`, `query`, `context` (for PostgreSQL/MySQL/SQLite).
...
6. **JSON parse errors** — Old PanDB versions may not support `--json` on some commands. If JSON output looks malformed, try without `--json` and parse the human-readable output instead.
