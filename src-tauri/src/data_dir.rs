...
const PORTABLE_MARKER: &str = "portable.pandb";
#[cfg(target_os = "windows")]
const LEGACY_PORTABLE_MARKER: &str = "portable.dbx";
#[cfg(target_os = "windows")]
...
pub fn resolve_data_dir_with_mode(default_app_data_dir: PathBuf) -> DataDirResolution {
    let env_data_dir = data_dir_override(std::env::var_os("PANDB_DATA_DIR"), std::env::var_os("DBX_DATA_DIR"));
...
fn portable_marker_exists(exe_dir: &Path) -> bool {
    exe_dir.join(PORTABLE_MARKER).is_file() || exe_dir.join(LEGACY_PORTABLE_MARKER).is_file()
}
...
mod tests {
    ...
    use super::{
        alternative_data_dir, data_dir_override, legacy_app_data_dir, resolve_data_dir_from_inputs, DataDirMode,
    };
