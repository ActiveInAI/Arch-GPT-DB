...
fn setup_desktop_tray<R: tauri::Runtime, M: Manager<R>>(
    ...
    let mut tray =
        TrayIconBuilder::<R>::with_id(DESKTOP_TRAY_ID).tooltip("Arch-GPT DB").menu(&menu).show_menu_on_left_click(false);
    #[cfg(target_os = "macos")]
    {
...
mod tests {
    ...
    fn startup_data_dir_diagnostics_never_include_paths() {
        let private_path = PathBuf::from(r"C:\Users\private-user\Arch-GPT DBData");
    ...
    fn appimage_linux_webkit_quirk_respects_x11_first_and_explicit_wayland() {
        let appimage = Some(OsStr::new("/opt/Arch-GPT DB.AppImage"));
    ...
    fn prefers_x11_for_appimage_wayland_when_backend_is_not_user_configured() {
        ...
        assert_eq!(linux_appimage_wayland_backend_override(Some(OsStr::new("/tmp/Arch-GPT DB.AppImage")), None, None), None);
