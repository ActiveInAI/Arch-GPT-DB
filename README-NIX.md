...
Add PanDB as a flake input and include the package in `environment.systemPackages`.
...
This method installs PanDB for a specific user through [Home Manager](https://github.com/nix-community/home-manager).
...
> PanDB ineligible for the upstream binary cache (it will be built locally).
...
If you are contributing to PanDB or building it from a local clone, the flake provides a fully configured development shell with Rust, Node.js, pnpm, and all GTK/WebKit system libraries:

```bash
git clone https://github.com/ActiveInAI/PanDB
cd dbx
nix develop
```
...
```bash
nix build github:ActiveInAI/PanDB#dbx-desktop
