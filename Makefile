...
help:
	...
	@printf '  %-23s %s\n' 'make db DB=mysql@8.4' 'Start and print PanDB connection fields'
...
check-tauri-dev-port:
	@if lsof -nP -iTCP:$(TAURI_DEV_PORT) -sTCP:LISTEN >/dev/null 2>&1; then \
		echo "Port $(TAURI_DEV_PORT) is already in use. PanDB Tauri dev requires http://localhost:$(TAURI_DEV_PORT)."; \
		echo ""; \
