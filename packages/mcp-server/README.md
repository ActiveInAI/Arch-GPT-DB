...
Rust-powered Model Context Protocol server for [PanDB](https://github.com/ActiveInAI/PanDB). It lets MCP-compatible AI agents inspect schemas and run safe database operations using connections configured in PanDB.

[中文说明](#中文说明) | [npm](https://www.npmjs.com/package/@dbx-app/mcp-server) | [Native releases](https://github.com/ActiveInAI/PanDB/releases?q=packages-v)
...
- **10 MCP tools** for connections, schemas, SQL, Redis, and PanDB UI integration
...
- **Agent/JDBC database support** through PanDB agent infrastructure when the required agent and JRE are installed
- **PanDB-managed access policy** with a connection allowlist and three execution modes
...
- **Optional desktop integration** for opening tables and displaying query results in PanDB
...
Every package release publishes native archives and `SHA256SUMS` in [GitHub Releases](https://github.com/ActiveInAI/PanDB/releases?q=packages-v):
...
Direct native execution does not require Node.js. GitHub package releases are intentionally not marked as the repository's latest release, so they do not replace the latest PanDB desktop release.
...
PanDB MCP reads connection profiles from PanDB storage. PanDB does not need to remain open for native connections. However:

- the connection must already exist in PanDB storage, unless it is added through `dbx_add_connection`;
- PanDB Agent/JDBC databases require the matching agent, JDBC driver, and JRE to be installed;
- `dbx_open_table` and `dbx_execute_and_show` require a running PanDB desktop application;
- PanDB Web mode requires a reachable PanDB Web server.

## Usage Examples

Ask the MCP client to:

- "List my PanDB connections"
...
- "Open the orders table in PanDB"
...
| `dbx_add_connection` | Add a connection to PanDB storage |
| `dbx_remove_connection` | Remove a connection from PanDB storage |
...
| `dbx_open_table` | Open a table in the running PanDB desktop application |
| `dbx_execute_and_show` | Execute a query and display the result in the PanDB desktop application |
...
This is the default. MCP reads PanDB connection storage and executes supported connections locally in the Rust process.

Common native paths include PostgreSQL, MySQL, SQLite, compatible SQL databases, Redis standalone, and MongoDB. SSH, cluster, vendor-specific, or Agent/JDBC connections may require additional PanDB infrastructure.

DuckDB runs through the standalone PanDB DuckDB driver. Install it from PanDB Driver Manager before using a DuckDB connection through local MCP. The MCP binary includes the sidecar client but does not bundle the DuckDB engine.

PanDB connection storage defaults to:

- macOS: `~/Library/Application Support/com.activeinai.pandb/dbx.db`
- Linux: `~/.local/share/com.activeinai.pandb/dbx.db`
- Windows: `%APPDATA%\com.activeinai.pandb\dbx.db`

Override the directory with `PANDB_DATA_DIR` (the legacy `DBX_DATA_DIR` is still supported). If the new directory has not been created, MCP continues to use an existing `com.dbx.app` data store.

### Agent/JDBC databases

Databases such as Dameng, Kingbase, Oracle, DB2, Hive, Trino, Snowflake, SAP HANA, and other PanDB Agent profiles use PanDB's Java agent infrastructure rather than a Node.js database driver.

The native npm/GitHub binary does not bundle every proprietary JDBC driver or JRE. Install the database agent through PanDB first, or provide a compatible agent installation under the PanDB agent directory. Availability depends on the installed driver and license terms of the database vendor.

### PanDB Web / Docker mode

Set `PANDB_WEB_URL` to use a deployed PanDB Web backend instead of local desktop storage:
...
```json
{
  "mcpServers": {
    "pandb": {
      ...
      "env": {
        "PANDB_WEB_URL": "https://pandb.example.com",
...
`PANDB_WEB_PASSWORD` is the password used on the PanDB Web login page. Desktop-local mode does not use it. Desktop UI tools are hidden in Web mode.

Docker and PanDB Web also require the standalone DuckDB driver. Install it from Driver Manager after the first launch; when `/app/data` is persisted, the driver is stored under `/app/data/agents` and survives container upgrades.

### Windows portable PanDB

Point `PANDB_DATA_DIR` at the portable `data` directory containing `dbx.db`:
...
```json
{
  "mcpServers": {
    "pandb": {
      "command": "pandb-mcp-server",
      "env": {
        "PANDB_DATA_DIR": "D:\\PanDB_x64-portable\\data"
      }
    }
  }
}
...
## PanDB-managed MCP Policy

PanDB stores one authoritative policy under **Settings → MCP** and reloads it for every request:
...
Legacy connection scope variables can still narrow the PanDB allowlist for existing client configurations:
...
Choose **Read only**, **Data read/write**, or **Full access** in PanDB instead of placing permission flags in client configuration. Updated servers do not let `PANDB_MCP_ALLOW_WRITES` or `PANDB_MCP_ALLOW_DANGEROUS_SQL` widen the PanDB policy. For upgrade compatibility, `PANDB_MCP_ALLOW_WRITES=0` (or `false`) keeps MCP read-only until a central policy is saved for the first time; the legacy permission variables are ignored afterward.
...
| Variable | Purpose |
| --- | --- |
| `PANDB_DATA_DIR` | Override the local PanDB data directory |
| `PANDB_WEB_URL` | Use a PanDB Web/Docker backend |
| `PANDB_WEB_PASSWORD` | Authenticate to the PanDB Web backend |
| `PANDB_MCP_ALLOW_WRITES` | Upgrade compatibility only: `0`/`false` keeps an unconfigured policy read-only |
...
Set `PANDB_DATA_DIR` to the directory containing `dbx.db`, not to the database file itself.

### Desktop action says PanDB is not running

Database queries can run without the desktop application when the connection is supported locally. `dbx_open_table` and `dbx_execute_and_show` intentionally require PanDB desktop to be running.

### Agent/JDBC database cannot start

Open PanDB Driver Manager and install/update the matching database agent and JRE. The standalone MCP binary does not redistribute every proprietary JDBC driver.
...
PanDB MCP Server 是 [PanDB](https://github.com/ActiveInAI/PanDB) 的 Rust MCP 服务，让 Claude Code、Cursor、Windsurf 等兼容 MCP 的 AI 工具使用 PanDB 中已有的连接查询数据库。

[npm](https://www.npmjs.com/package/@dbx-app/mcp-server) | [原生版本下载](https://github.com/ActiveInAI/PanDB/releases?q=packages-v)
...
每个 packages 版本会在 [GitHub Releases](https://github.com/ActiveInAI/PanDB/releases?q=packages-v) 发布以下文件：
...
- 连接配置需要存在于 PanDB 存储中，或通过 `dbx_add_connection` 添加
- Agent/JDBC 数据库需要提前安装对应 Agent、JDBC Driver 和 JRE
- `dbx_open_table`、`dbx_execute_and_show` 需要 PanDB 桌面端正在运行
...
| `dbx_add_connection` | 添加 PanDB 连接配置 |
| `dbx_remove_connection` | 删除 PanDB 连接配置 |
...
| `dbx_open_table` | 在 PanDB 桌面端打开表 |
| `dbx_execute_and_show` | 执行查询并在 PanDB 桌面端展示结果 |
...
通过 `PANDB_DATA_DIR` 覆盖默认目录（旧 `DBX_DATA_DIR` 仍兼容）。如果新目录尚未创建，MCP 会继续使用已有的 `com.dbx.app` 数据目录。Windows 便携版应指向 `PanDB.exe` 同级、包含 `dbx.db` 的 `data` 文件夹。
...
Web 模式不会读取本机 PanDB 桌面存储，也不会暴露桌面 UI 工具。

### Agent/JDBC 数据库

达梦、人大金仓、Oracle、DB2、Hive、Trino、Snowflake、SAP HANA 等数据库通过 PanDB Java Agent/JDBC 基础设施运行，而不是通过 Node.js 数据库驱动运行。

npm 和 GitHub Release 中的原生 MCP 文件不会捆绑所有厂商的专有 JDBC Driver。请先通过 PanDB Driver Manager 安装对应 Agent 和 JRE，或提供兼容的 PanDB Agent 目录。

### PanDB 管理的 MCP 策略

PanDB 在 **设置 → MCP** 中保存一份权威策略，并在每次请求时重新读取：
...
旧连接 scope 变量可继续兼容读取，但只能进一步收窄 PanDB allowlist：
...
请在 PanDB 中选择 **只读**、**数据读写** 或 **完全访问**，不要在客户端配置中放置权限开关。新版 Server 不允许 `PANDB_MCP_ALLOW_WRITES` 或 `PANDB_MCP_ALLOW_DANGEROUS_SQL` 放宽 PanDB 中央策略。为兼容升级，在中央策略首次保存前，`PANDB_MCP_ALLOW_WRITES=0`（或 `false`）仍会保持 MCP 只读；策略保存后旧权限变量即被忽略。
...
| 变量 | 用途 |
| --- | --- |
| `PANDB_DATA_DIR` | 覆盖本地 PanDB 数据目录 |
| `PANDB_WEB_URL` | 使用 PanDB Web/Docker 后端 |
| `PANDB_WEB_PASSWORD` | PanDB Web 登录密码 |
| `PANDB_MCP_ALLOW_WRITES` | 仅用于升级兼容：`0`/`false` 使尚未配置的策略保持只读 |
...
将 `PANDB_DATA_DIR` 设置为包含 `dbx.db` 的目录，而不是数据库文件路径。

**提示 PanDB 未运行**

普通数据库查询不一定需要启动 PanDB；只有桌面 UI 工具和仍需 bridge 的连接需要 PanDB 运行。

**Agent 数据库无法启动**

通过 PanDB Driver Manager 安装或更新对应数据库 Agent、JDBC Driver 和 JRE。
